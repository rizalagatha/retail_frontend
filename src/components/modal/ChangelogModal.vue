...
interface Changelog {
  ...
  date: string;
  type: string;
  changes: (string | ChangelogDetail)[]; // Bisa berupa string biasa ATAU object detail
}
...
const props = defineProps({
  modelValue: Boolean,
  items: {
    // [PERBAIKAN 2] Ganti any[] menjadi Changelog[]
    type: Array as () => Changelog[],
    default: () => [],
  },
  loading: Boolean,
});
          ...
          >
            <div class="mb-4">
              <div class="font-weight-bold text-h6">
                v{{ log.version }}
                <span v-if="log.date !== '-'" class="text-caption text-grey ml-2"
                  >({{ log.date }})</span
                >
              </div>
                ...
                <li v-for="(change, idx) in log.changes" :key="idx" class="mb-1">
                  ...
                  <template v-else>
                    ...
                    <ul class="pl-4" style="list-style-type: circle">
                      <li
                        v-for="(subItem, subIdx) in change.items"
                        :key="subIdx"
