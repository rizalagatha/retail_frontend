...
const fetchPrintData = async (nomor: string) => {
  ...
  try {
    ...
    const fallbackSubTotal = details.reduce((sum, d) => sum + d.harga_asli * d.invd_jumlah, 0);

    const fallbackDiskon = details.reduce((sum, d) => sum + d.total_diskon, 0);
    ...
    <div v-else-if="printData">
      ...
      <div class="items">
        <div v-for="item in printData.details" :key="item.invd_kode" class="item">
          ...
          <template v-else>
            <div class="item-details">
              <span> {{ item.invd_jumlah }} x {{ formatRupiah(item.harga_setelah_diskon) }} </span>
              <span>{{ formatRupiah(item.total) }}</span>
            </div>
          </template>
        </div>
      ...
      <div class="summary" v-if="printData.details.length">
        ...
        <div class="summary-item">
          <span>Ppn</span><span>{{ formatRupiah(printData.header.summary.ppn) }}</span>
        </div>
        <div class="summary-item">
        ...
        <div class="summary-item">
          <span>Dp</span><span>{{ formatRupiah(printData.header.summary.dp) }}</span>
        </div>
        ...
        <div class="summary-item">
          <span>Pundi Amal</span><span>{{ formatRupiah(printData.header.summary.pundiAmal) }}</span>
        </div>
        <div class="summary-item">
