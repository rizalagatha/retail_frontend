...
const handleKodeSearch = async () => {
  ...
  try {
    ...
    if (data) { // Pastikan data tidak null
      ...
      toast.info(`Kode user ${kode.value} di cabang ${cabang.value} tidak ditemukan. Silakan isi data baru.`);
    }
  } catch (error: unknown) {
...
<template>
  <PageLayout title="Master User" icon="mdi-account-group" desktop-mode :loading="isLoading">
    <template #header-actions>
      ...
      <v-btn v-if="authStore.can(MENU_ID, 'insert') || authStore.can(MENU_ID, 'edit')" size="small" color="primary"
        @click="saveUser" :loading="isLoading">Simpan</v-btn>
    </template>
        ...
        <v-row dense>
          <v-col cols="12" sm="6" md="3">
            <v-text-field v-model="kode" label="Kode" placeholder="Ketik atau F1..." variant="outlined"
              density="compact" hide-details @keydown.enter.prevent="handleKodeSearch" @blur="handleKodeSearch"
              @keydown.f1.prevent="openHelpModal" :disabled="!isNewUser">
          ...
          <v-col cols="12" sm="6" md="3">
            <v-text-field v-model="password" label="Password" type="password"
              :placeholder="isNewUser ? 'Wajib diisi' : 'Isi untuk ganti'" variant="outlined" density="compact"
              hide-details></v-text-field>
          </v-col>
          <v-col cols="12" sm="6" md="3">
            <v-select v-model="cabang" :items="branches" item-title="gdg_kode" item-value="gdg_kode" label="Cabang"
              variant="outlined" density="compact" hide-details
              :disabled="!isNewUser || authStore.user?.cabang !== 'KDC'"
              @update:modelValue="handleKodeSearch"></v-select>
          </v-col>
        </v-row>
      </div>
      ...
      <div class="desktop-form-section flex-grow-1 d-flex flex-column">
        ...
        <v-data-table :items="permissions" :headers="[
          ...
          <template v-slot:[`item.view`]="{ item }">
            <div class="d-flex justify-center"><v-checkbox-btn v-model="item.view" hide-details density="compact"
                color="primary"></v-checkbox-btn></div>
          </template>
          <template v-slot:[`item.insert`]="{ item }">
            <div class="d-flex justify-center"><v-checkbox-btn v-model="item.insert" hide-details density="compact"
                color="success"></v-checkbox-btn></div>
          </template>
          <template v-slot:[`item.edit`]="{ item }">
            <div class="d-flex justify-center"><v-checkbox-btn v-model="item.edit" hide-details density="compact"
                color="warning"></v-checkbox-btn></div>
          </template>
          <template v-slot:[`item.delete`]="{ item }">
            <div class="d-flex justify-center"><v-checkbox-btn v-model="item.delete" hide-details density="compact"
                color="error"></v-checkbox-btn></div>
          </template>
        </v-data-table>
      </div>
    </div>
