...
const fetchData = async () => {
    ...
    selected.value = [];
    try {
        const response = await api.get('/jenis-kain');
...
// Fungsi ini akan dipanggil setelah modal berhasil menyimpan
const onDataSaved = () => {
    fetchData(); // Muat ulang data di grid
};
...
<template>
    <PageLayout title="Browse Jenis Kain" icon="mdi-texture-box">
        ...
        <MasterSimpleFormModal v-if="isFormModalVisible" title="Tambah Jenis Kain Baru" apiUrl="/jenis-kain"
            label1="Kode" field1="Kode" label2="Jenis Kain" field2="JenisKain" @close="isFormModalVisible = false"
            @saved="onDataSaved" />
