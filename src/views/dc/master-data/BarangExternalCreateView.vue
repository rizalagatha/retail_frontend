...
const dialogConfirm = reactive({
  ...
  text: '',
  onConfirm: () => { },
  onCancel: () => { dialogConfirm.show = false; } // Tambahkan onCancel
});
      ...
      <div class="left-column">
        <div class="desktop-form-section header-section">
          <v-row dense class="hide-details">
            ...
            <v-col cols="6"><v-text-field label="Date Create" v-model="header.date_create" type="date" readonly filled
                density="compact" /></v-col>

            <v-col cols="12"><v-select label="Kategori Produk" v-model="header.brg_ktgp" :items="ktgProdukOptions"
                variant="outlined" density="compact" /></v-col>
            <v-col cols="12"><v-select label="Kategori Barang" v-model="header.brg_ktg" :items="kategoriOptions"
                variant="outlined" density="compact" :readonly="isEditMode" /></v-col>

            <v-col cols="12"><v-text-field label="Nama Barang" v-model="header.nama" variant="outlined"
                density="compact" /></v-col>
            <v-col cols="12"><v-text-field label="Bahan" v-model="header.bahan" variant="outlined"
                density="compact" /></v-col>

            <v-col cols="12"><v-radio-group v-model="header.brg_aktif" inline label="Status" density="compact">
                <v-radio label="Aktif" :value="0" />
                <v-radio label="Pasif" :value="1" />
              </v-radio-group></v-col>

            <v-col cols="12"><v-text-field label="ID Barcode" v-model="header.brg_bcdid" variant="outlined"
                density="compact" /></v-col>

            <v-col cols="12">
              <v-img v-if="header.imageUrl" :src="header.imageUrl" height="150" contain class="border" />
              <div v-else class="border d-flex align-center justify-center"
                style="height: 150px; background-color: #f5f5f5;">(No Image)</div>
              <v-file-input label="Upload Gambar (Max 1MB)" @change="onFileSelect" variant="underlined"
                density="compact" clearable @click:clear="clearImage" />
            </v-col>
          </v-row>
        </div>
      </div>
