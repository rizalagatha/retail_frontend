...
const pageTitle = computed(() =>
  isEditMode.value ? "Ubah Setoran Pembayaran" : "Buat Setoran Pembayaran"
);
const requiredPermission = computed(() => (isEditMode.value ? "edit" : "insert"));
const cabangUser = computed(() => authStore.user?.cabang ?? "");
...
const handleLunasi = (item: Item) => {
  if (item.lunasi) {
    const sisaSetoran =
      header.nominal -
      items.value.filter((i) => i.id !== item.id).reduce((sum, i) => sum + (i.bayar || 0), 0);
...
const handleSave = () => {
  ...
  if (header.sisa < 0)
    return toast.error("Sisa setoran tidak boleh minus. Periksa kembali alokasi pembayaran.");
...
onMounted(() => {
  ...
  if (!authStore.can(MENU_ID, requiredPermission.value)) {
    toast.error(
      `Anda tidak memiliki izin untuk ${isEditMode.value ? "mengubah" : "membuat"} data Setoran.`
    );
    router.push({ name: "SetoranBayar" });
    return;
  }
...
<template>
  <PageLayout :title="pageTitle" desktop-mode icon="mdi-cash-multiple">
          ...
          <v-row dense>
            ...
            <v-col cols="6">
              ...
              >
                <template #append-inner
                  ><span v-if="!isEditMode" class="text-caption text-disabled"
                    >&lt;Baru&gt;</span
                  ></template
                >
              </v-text-field>
            ...
            <v-col cols="12">
              <v-text-field
                ...
                @click="
                  () => {
                    if (!lockSoMode) dialog.customerSearch = true;
                  }
                "
            ...
            <v-col cols="12">
              ...
              >
                ...
                <v-radio label="QRIS" value="QRIS"></v-radio>
            ...
            <v-expand-transition>
              <div
                v-if="header.jenisSetor === 'TRANSFER' || header.jenisSetor === 'QRIS'"
                class="w-100"
              >
                <v-row dense>
                  <v-col cols="6">
                  ...
                  <v-col cols="12">
                    <v-text-field
                      label="Tgl. Transaksi"
                      v-model="header.tanggalTransfer"
                      type="date"
                      variant="outlined"
                      density="compact"
                      hide-details
              ...
              <div v-if="header.jenisSetor === 'GIRO'" class="w-100">
                <v-row dense>
                  ...
                  <v-col cols="6">
                    <v-text-field
                      label="Jatuh Tempo"
                      v-model="header.tanggalJatuhTempo"
                      type="date"
                      variant="outlined"
                      density="compact"
                      hide-details
            ...
            <v-row dense>
              ...
              <v-col cols="12" md="6" v-if="header.nomorSo">
                <v-text-field
                  :model-value="formatRupiah(header.minimalDp)"
                  :label="`Minimal DP (${header.minimalPercent || 30}%)`"
                  readonly
                  filled
                  density="compact"
                  hide-details
              ...
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="
                    focusedRowId === 'header_nominal'
                      ? header.nominal
                      : formatRupiah(header.nominal)
                  "
                  @focus="onHeaderNominalFocus"
                  @blur="onHeaderNominalBlur"
                  @update:model-value="onHeaderNominalUpdate"
              ...
              <v-col cols="6" md="3">
                <v-text-field
                  :model-value="formatRupiah(header.terbayar)"
                  label="Terbayar"
                  readonly
                  filled
                  density="compact"
              ...
              <v-col cols="6" md="3">
                <v-text-field
                  :model-value="formatRupiah(header.sisa)"
                  label="Sisa"
                  readonly
                  filled
                  density="compact"
        ...
        <div class="desktop-form-section d-flex flex-column fill-height">
          ...
          >
            ...
            <template #[`item.bayar`]="{ item }">
              <v-text-field
                :model-value="focusedRowId === item.id ? item.bayar : formatRupiah(item.bayar)"
                @focus="focusedRowId = item.id"
                @blur="focusedRowId = -1"
                @update:model-value="
                  item.bayar = Number(String($event).replace(/[^0-9]/g, '')) || 0;
                  calculateTotals();
                "
                variant="underlined"
                density="compact"
    ...
    <CustomerSearchModal
      v-if="dialog.customerSearch"
      :gudang="authStore.user?.cabang || ''"
      @close="dialog.customerSearch = false"
      @customer-selected="onCustomerSelected"
    />
    <RekeningSearchModal
      v-if="dialog.rekeningSearch"
      :cabang="authStore.user?.cabang || ''"
      @close="dialog.rekeningSearch = false"
      @selected="onRekeningSelected"
    />
    <UnpaidInvoiceSearchModal
      v-if="dialog.invoiceSearch"
      :customer-kode="header.customer.kode"
      @close="dialog.invoiceSearch = false"
      @invoice-selected="onUnpaidInvoiceSelected"
    />
    <SoSearchModal
      v-if="isSoSearchVisible"
      :cabang="authStore.user?.cabang || ''"
      source="setoran-bayar"
    ...
    <InvoiceSearchModal
      ...
      source="biaya-kirim"
      :customer-kode="header.customer.kode"
      @close="dialogBk = false"
      @invoice-selected="onBkSelected"
    />
