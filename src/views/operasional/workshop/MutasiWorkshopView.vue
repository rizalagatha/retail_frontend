...
const MENU_ID = "801"; // [PERBAIKAN] Sesuaikan dengan MENU_ID yang kita sepakati
...
const fetchCabangList = async () => {
  try {
    ...
    // [PERBAIKAN] Suntikkan opsi "ALL STORE" jika usernya KDC atau W01
...
const fetchMasterData = async () => {
  ...
  details.value = {};
  try {
    // [UBAH] API ENDPOINT KE WORKSHOP
    const response = await api.get("/mutasi-workshop", { params: filters });
...
const loadDetails = async (newlyExpandedItems: MasterDataItem[]) => {
  ...
  try {
    // [UBAH] API ENDPOINT KE WORKSHOP
    const response = await api.get(`/mutasi-workshop/details/${itemToLoad.nomor}`);
...
const handleDelete = () => {
  ...
  showConfirmation("Konfirmasi Hapus", `Yakin ingin menghapus dokumen ${nomor}?`, async () => {
    try {
      // [UBAH] API ENDPOINT KE WORKSHOP
      const response = await api.delete(`/mutasi-workshop/${nomor}`);
...
const handleNew = () => {
  router.push({ name: "MutasiWorkshopCreate" }); // [UBAH] Route Name
};

const handleEdit = () => {
  if (!isSingleSelected.value) return;
  const nomor = selected.value[0].nomor;
  router.push({ name: "MutasiWorkshopEdit", params: { nomor } }); // [UBAH] Route Name
};

const handlePrint = () => {
  if (!selectedRow.value) return;
  const url = router.resolve({
    name: "MutasiWorkshopPrint", // [UBAH] Route Name
...
const exportData = async (type: "header" | "detail") => {
  if (type === "header") {
    ...
    try {
      ...
      XLSX.writeFile(workbook, "Export_Mutasi_Workshop_Header.xlsx");
    ...
    try {
      ...
      // [UBAH] API ENDPOINT KE WORKSHOP
      const response = await api.get<MutasiKirimExportRow[]>("/mutasi-workshop/export-details", {
        params: filters,
      });
      ...
      const title = "LAPORAN MUTASI KE WORKSHOP";
      ...
      XLSX.writeFile(workbook, "Export_Mutasi_Workshop_Detail.xlsx");
...
<template>
  <PageLayout title="Mutasi ke Workshop" :icon="'mdi-sewing-machine'">
