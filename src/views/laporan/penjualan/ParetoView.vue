...
const fetchKategoriOptions = async () => {
    ...
    } catch (error) { toast.error('Gagal memuat filter kategori.'); }
};
        ...
        <div class="browse-content">
            <div class="filter-section">
                ...
                <v-select v-model="filters.cabang" :items="cabangOptions" item-title="nama" item-value="kode"
                    label="Gudang" density="compact" hide-details variant="outlined" class="ms-4"
                    style="max-width: 180px;" />
            ...
            <div class="table-container"> <v-data-table :headers="headers" :items="items" :loading="isLoading"
                    ...
                    <template v-for="col in headers" #[`item.${col.key}`]="{ item }">
                        <td>{{ item[col.key]?.toLocaleString('id-ID') }}</td>
                    </template>
