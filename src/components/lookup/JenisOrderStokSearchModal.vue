...
const loadItems = async () => {
  loading.value = true;
  try {
    // Panggil API khusus untuk Jenis Order Stok
    const response = await api.get("/so-dtf-stok-form/lookup/jenis-order-stok", {
      params: { term: search.value },
    });
    items.value = response.data;
  } catch (error) {
    console.error("Gagal memuat data jenis order stok:", error);
    items.value = [];
  } finally {
    loading.value = false;
  }
  ...
  <v-dialog :model-value="true" @update:model-value="$emit('close')" max-width="800px" persistent>
    <v-card class="dialog-card d-flex flex-column" style="height: 60vh">
      <v-toolbar color="primary" density="compact">
        <v-toolbar-title class="text-subtitle-1">Bantuan - Pilih Jenis Order Stok</v-toolbar-title>
        <v-spacer></v-spacer>
