...
const selectedRow = computed<MutasiHeader | null>(() =>
...
const handleEdit = () => {
  if (!canEdit.value) {
    if (selectedRow.value?.noSTBJ)
      return toast.error("Mutasi Otomatis dari STBJ. Tidak bisa diubah.");
    if (selectedRow.value?.closing === "Y")
      return toast.error("Sudah Closing Stok Opname. Tidak bisa diubah.");
    return;
  }
...
const openDeleteDialog = () => {
  if (!canDelete.value) {
    if (selectedRow.value?.noSTBJ)
      return toast.error("Mutasi Otomatis dari STBJ. Tidak bisa dihapus.");
    if (selectedRow.value?.closing === "Y")
      return toast.error("Sudah Closing Stok Opname. Tidak bisa dihapus.");
    return;
  }
  showConfirmation(
    "Konfirmasi Hapus",
    `Yakin ingin hapus mutasi ${selectedRow.value!.nomor}?`,
    handleDelete
  );
};
...
<template>
  <PageLayout title="Mutasi Stok Antar Gudang" :menu-id="MENU_ID">
    ...
    <div class="browse-content">
      <div class="filter-section">
        ...
        <v-text-field
          ...
          readonly
          @click="isBarangSearchVisible = true"
          @keydown.f1.prevent="isBarangSearchVisible = true"
          clearable
          @click:clear="clearBarangFilter"
          append-inner-icon="mdi-magnify"
          @click:append-inner="isBarangSearchVisible = true"
        />
      ...
      <div class="table-container">
        ...
        >
          ...
          <template #[`item.ngedit`]="{ item }">
            <v-chip
              v-if="item.ngedit"
              :color="getStatusChipColor(item.ngedit as string)"
              size="x-small"
              class="pa-1"
            >
              {{ item.ngedit }}
    ...
    <MintaBarangSearchModal
      ...
      @product-selected="onBarangSelected"
    />
    ...
    <v-dialog v-model="dialogAlasan.show" max-width="500px" persistent>
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="handleAjukan" :loading="dialogAlasan.isLoading"
            >Ajukan</v-btn
          >
        </v-card-actions>
      </v-card>
