...
const loadFromSO = async () => {
    ...
    try {
        ...
        // --- Copy Detail Items ---
        items.value = soItems.map((item: any) => ({ ...item, id: Math.random() }));
...
const tableHeaders = [
    ...
    { title: 'Jumlah', key: 'jumlah', sortable: false, align: 'end', width: '50px' },
    { title: 'Harga', key: 'harga', sortable: false, align: 'end', width: '80px' },
    { title: 'Diskon %', key: 'diskonPersen', sortable: false, align: 'end', width: '60px' },
    { title: 'Diskon Rp', key: 'diskonRp', sortable: false, align: 'end', width: '60px' },
    { title: 'Total', key: 'total', sortable: false, align: 'end', width: '80px' },
    { title: 'Barcode', key: 'barcode', sortable: false, width: '90px' }, // <-- TAMBAHKAN INI
];
    ...
    <PageLayout :title="isEditMode ? 'Ubah Proforma Invoice' : 'Buat Proforma Invoice'" :menu-id="MENU_ID">
        ...
        <div v-if="!loading" class="form-grid-container">
            ...
            <div class="left-column">
                <div class="desktop-form-section header-section">
                    <v-row dense>
                        <v-col cols="6"><v-text-field label="Cabang" v-model="formHeader.cabang" readonly filled
                                density="compact" hide-details /></v-col>
                        <v-col cols="6"><v-text-field label="Nomor" v-model="formHeader.nomor" readonly filled
                                density="compact" hide-details /></v-col>
                        <v-col cols="6"><v-text-field label="Tanggal" v-model="formHeader.tanggal" type="date"
                                density="compact" hide-details /></v-col>
                        <v-col cols="6">
                            <v-text-field label="No. SO (F1)" v-model="formHeader.nomorSo" :readonly="isEditMode"
                                density="compact" hide-details @keydown.enter="loadFromSO"
                                @keydown.f1.prevent="isSoLookupVisible = true" placeholder="F1 untuk cari..."
                                append-inner-icon="mdi-magnify" @click:append-inner="isSoLookupVisible = true" />
                        </v-col>
                    </v-row>
                </div>
                <div class="desktop-form-section header-section">
                    <v-row dense>
                        <v-col cols="12">
                            <v-text-field label="Customer (F1)" v-model="formHeader.customerNama"
                                append-inner-icon="mdi-magnify" @click:append-inner="isCustomerLookupVisible = true"
                                @keydown.f1.prevent="isCustomerLookupVisible = true" readonly filled density="compact"
                                hide-details />
                        </v-col>
                        <v-col cols="12"><v-text-field label="Alamat" v-model="formHeader.alamat" readonly filled
                                density="compact" hide-details /></v-col>
                        <v-col cols="6"><v-text-field label="Level" v-model="formHeader.level" readonly filled
                                density="compact" hide-details /></v-col>
                        <v-col cols="6"><v-text-field label="Kota" v-model="formHeader.kota" readonly filled
                                density="compact" hide-details /></v-col>
                    </v-row>
                </div>
                <div class="desktop-form-section header-section">
                    <v-row dense>
                        <v-col cols="4"><v-text-field label="TOP (hari)" v-model.number="formHeader.top"
                                variant="outlined" density="compact" hide-details /></v-col>
                        <v-col cols="8"><v-text-field label="Jatuh Tempo" v-model="formHeader.tempo" type="date"
                                readonly filled density="compact" hide-details /></v-col>
                        <v-col cols="12"><v-textarea label="Keterangan" v-model="formHeader.keterangan"
                                variant="outlined" density="compact" hide-details rows="2" /></v-col>
                    </v-row>
                </div>
            </div>
            ...
            <div class="right-column">
                <div class="table-container">
                    <v-data-table :headers="tableHeaders" :items="items" class="desktop-table fill-height"
                ...
                <div class="desktop-form-section">
                    <v-row dense>
                        <v-col><v-text-field label="Subtotal"
                                :model-value="totalSetelahDiskonItem.toLocaleString('id-ID')" readonly filled
                                density="compact" hide-details class="text-right" /></v-col>
                        <v-col><v-text-field label="Diskon (%)" v-model.number="summary.diskonPersen" density="compact"
                                hide-details class="text-right" /></v-col>
                        <v-col><v-text-field label="Diskon (Rp)" v-model.number="summary.diskon" density="compact"
                                hide-details class="text-right" /></v-col>
                        <v-col><v-text-field label="PPN (%)" v-model.number="formHeader.ppn" density="compact"
                                hide-details class="text-right" style="max-width: 100px" /></v-col>
                        <v-col><v-text-field label="Total PPN" :model-value="totalPpn.toLocaleString('id-ID')" readonly
                                filled density="compact" hide-details class="text-right" /></v-col>
                    </v-row>
                    <v-row dense class="mt-2">
                        <v-col><v-text-field label="Grand Total" :model-value="grandTotal.toLocaleString('id-ID')"
                                readonly filled density="compact" hide-details
                                class="text-right font-weight-bold" /></v-col>
                        <v-col><v-text-field label="Biaya Kirim" v-model.number="summary.biayaKirim" density="compact"
                                hide-details class="text-right" /></v-col>
        ...
        <v-dialog v-model="dialogConfirmSave.show" max-width="400px" persistent>
            <v-card>
                ...
                <v-card-text v-html="dialogConfirmSave.text"></v-card-text>
                <v-card-actions>
                    <v-spacer />
        ...
        <v-dialog v-model="dialogConfirmPrint.show" max-width="400px" persistent>
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer />
                    <v-btn text
                        @click="() => { dialogConfirmPrint.onCancel(); dialogConfirmPrint.show = false; }">Tutup</v-btn>
                    <v-btn color="primary"
