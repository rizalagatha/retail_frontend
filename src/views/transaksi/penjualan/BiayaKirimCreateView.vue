...
const loadInvoiceDetail = async (inv: BiayaKirimLookupInvoice) => {
  ...
  try {
    ...
    toast.error(err.message || "Gagal ambil detail invoice");
  }
};
      ...
      <v-row dense>
        <v-col cols="12" md="6">
          <v-card variant="outlined" class="pa-4 custom-border">
            ...
            <v-text-field
              v-model="form.inv_nomor"
              label="Nomor Invoice (F1)"
              prepend-inner-icon="mdi-magnify"
              readonly
              @click="isNew && (dialogs.invoiceSearch = true)"
              density="compact"
            />
            <v-row dense>
              <v-col cols="6"
                ><v-text-field
                  v-model="form.inv_tanggal"
              ...
              <v-col cols="6"
                ><v-text-field
                  :model-value="formatRupiah(form.nominal_inv)"
    ...
    <v-dialog v-model="dialogs.printOption" max-width="400" persistent>
      <v-card class="text-center pa-4">
        ...
        <v-card-text
          ...
          ><br />Ingin cetak bukti transaksi ini?</v-card-text
        >
