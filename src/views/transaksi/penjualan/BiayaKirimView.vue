...
onMounted(async () => {
  ...
  if (authStore.user?.cabang === 'KDC') cabangList.value.unshift({ kode: 'ALL', nama: 'Semua Cabang' });
  await fetchData();
      ...
      <div class="table-container">
        <v-data-table-server v-model="selected" v-model:expanded="expanded" v-model:page="options.page"
          ...
          <template #[`item.SisaPiutang`]="{ value }">
            <span :class="value !== 0 ? 'bg-red text-white px-2 rounded' : ''">{{ formatRupiah(value) }}</span>
          </template>
            ...
            <tr>
              <td :colspan="columns.length" class="pa-0 bg-blue-lighten-5">
                <div class="py-4 px-12">
                  ...
                  <v-data-table :headers="detailHeaders" :items="details[item.Nomor] || []"
                    :loading="loadingDetails.has(item.Nomor)" density="compact" hide-default-footer
