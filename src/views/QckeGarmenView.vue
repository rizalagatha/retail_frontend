...
const getRowProps = ({ item }: { item: QCMaster }) => {
    return {
        class: item.Close === 'N' ? 'row-open-status' : ''
    };
};
...
const handleDelete = async () => {
    ...
    if (confirm(`Yakin ingin hapus data nomor ${nomor}?`)) {
        try {
            ...
            toast.error(`Gagal menghapus data ${nomor}.`);
        }
    }
};
        ...
        <v-card flat>
            <v-card-title class="d-flex align-center pe-2">
                ...
                <div class="d-flex ga-2">
                    ...
                    <v-btn size="small" color="green" prepend-icon="mdi-printer" :disabled="!isSingleSelected" @click="handlePrint">
                        Cetak
                    </v-btn>
                </div>
            </v-card-title>
            ...
            <v-data-table
                ...
                @update:expanded="({ item, value }) => loadDetails({ item, value })"
            >
                ...
                <template #item.Close="{ item }">
                    <v-chip :color="item.Close === 'Y' ? 'success' : 'orange'" size="x-small">
