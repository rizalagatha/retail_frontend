...
const cabangOptions = ref<Option[]>([]);
const kategoriOptions = ref<Option[]>([]);
const page = ref(1);
...
const filters = reactive<{
  ...
  cabang: authStore.user?.cabang === "KDC" ? "ALL" : authStore.user?.cabang ?? null,
  kategori: "ALL",
  search: "",
  limit: 50,
...
const filteredItems = computed(() => {
  ...
  for (const key in columnFilters.value) {
    ...
    if (f.type === "multi") {
      data = data.filter((r) => {
        const val = r[key];
        return val !== undefined && f.values!.includes(val);
      });
    }
...
const fetchKategoriOptions = async () => {
  try {
    ...
    const error = err as AxiosError<{ message?: string }>;
    toast.error(error.response?.data?.message || "Gagal memuat filter kategori.");
  }
};
const uniqueValues = (key: string): FilterValue[] => {
  const set = new Set<FilterValue>(
    items.value
      .map((i) => i[key])
      .filter((v): v is FilterValue => v !== null && v !== undefined && v !== "")
  );
...
const exportToExcel = async () => {
  ...
  // Export persis apa yang sedang tampil di layar (filteredItems, sesuai filter kolom aktif),
  // satu sheet, kolom Cab ikut hanya kalau memang sedang ditampilkan di tabel.
  const showCabColumn = filters.cabang !== "ALL" || filters.detailPerCabang;
  const rows = filteredItems.value;
  if (rows.length === 0) return toast.warning("Tidak ada data untuk diekspor (cek filter kolom).");

  const keys = Object.keys(rows[0]).filter((k) => showCabColumn || k !== "Cab");

  const workbook = new ExcelJS.Workbook();
  const sheet = workbook.addWorksheet("Pareto");
  ...
  const cabLabel =
    filters.cabang === "ALL"
      ? filters.detailPerCabang
        ? "PerCabang"
        : "Gabungan"
      : filters.cabang;
  a.download = `Laporan_Pareto_${cabLabel}_${filters.startDate}_${filters.endDate}.xlsx`;
  a.click();
...
<template>
  <PageLayout title="Laporan Pareto Barang Terjual" :menu-id="MENU_ID">
    <template #header-actions>
      <v-btn size="small" color="primary" @click="handlePrint" prepend-icon="mdi-printer"
        >Cetak</v-btn
      >
