...
const cabangHeaders = [{ title: 'Cabang', key: 'cab' }, { title: 'Berlaku', key: 'berlaku', align: 'center' }];
const levelHeaders = [{ title: 'Kode', key: 'kode' }, { title: 'Level Customer', key: 'level' }, { title: 'Berlaku', key: 'berlaku', align: 'center' }];
...
const onBonusSelected = (products: any[]) => {
    isBonusSearchVisible.value = false;
    const selected = products[0];
    if (!selected) return;
    const isDuplicate = bonusItems.value.some(b => b.kode === selected.kode && b.ukuran === selected.ukuran);
...
const loadDataForEdit = async (nomor: string) => {
    ...
    try {
        ...
        applicableItems.value = data.applicableItems.map((item: any) => ({ ...item, id: Math.random() }));
        bonusItems.value = data.bonusItems.map((item: any) => ({ ...item, id: Math.random() }));
...
const onApplicableSelected = (products: any[]) => {
    isApplicableSearchVisible.value = false;
    const selected = products[0];
    if (!selected) return;
    const isDuplicate = applicableItems.value.some(item => item.kode === selected.kode && item.ukuran === selected.ukuran);
...
<template>
    <PageLayout :title="pageTitle" desktop-mode icon="mdi-gift">
                    ...
                    <div class="header-main">
                        <div class="desktop-form-section fill-height">
                            ...
                            <v-row dense class="compact-form">
                                <v-col cols="12">
                                    <v-text-field label="Nomor Promo" v-model="header.nomor" readonly filled
                                ...
                                <v-col cols="6">
                                    <v-text-field label="Tanggal Mulai" v-model="header.tanggal1" type="date"
                                        variant="outlined" hide-details density="compact" style="padding-right: 2px;" />
                                </v-col>

                                <v-col cols="6">
                                    <v-text-field label="Tanggal Selesai" v-model="header.tanggal2" type="date"
                                        variant="outlined" hide-details density="compact" style="padding-left: 2px;" />
                                </v-col>

                                <v-col cols="12">
                                    <v-radio-group v-model="header.jenis" inline label="Jenis Promo" hide-details
                                        density="compact">
                                ...
                                <v-col cols="6">
                                    <v-text-field label="Total Qty Belanja" v-model.number="header.totalQty"
                                        type="number" variant="outlined" :disabled="header.jenis !== 2" hide-details
                                        density="compact" style="padding-left: 2px;" />
                                </v-col>

                                <v-col cols="12">
                                    <v-select label="Berlaku Kelipatan" v-model="header.kelipatan" :items="['Y', 'N']"
                                        variant="outlined" hide-details density="compact" />
                                </v-col>

                                <v-col cols="12">
                                    <v-textarea label="Keterangan" v-model="header.keterangan" rows="3"
                                        variant="outlined" hide-details density="compact" />
                                </v-col>

                                <v-col cols="12">
                                    <v-textarea label="Note (muncul di struk)" v-model="header.note" rows="3"
                                        variant="outlined" hide-details density="compact" />
                                </v-col>
                            </v-row>
                        </div>
                    ...
                    <div class="sub-sections">
                        <div class="top-row">
                            <div class="desktop-form-section fill-height">
                                ...
                                <v-data-table :headers="cabangHeaders" :items="cabangList" class="desktop-table"
                                    :items-per-page="-1" density="compact" fixed-header>
                                    <template #item.berlaku="{ item }"><v-checkbox-btn v-model="item.berlaku"
                            ...
                            <div class="desktop-form-section fill-height">
                                ...
                                <v-data-table :headers="levelHeaders" :items="levelList" class="desktop-table"
                                    :items-per-page="-1" density="compact" fixed-header>
                                    <template #item.berlaku="{ item }"><v-checkbox-btn v-model="item.berlaku"
                                            hide-details /></template>
                                    <template #bottom></template>
                        ...
                        <div class="bottom-row">
                            <div class="desktop-form-section fill-height">
                                ...
                                <v-row dense class="compact-form">
                                    <v-col cols="6">
                                        <v-text-field label="Diskon Faktur Rp" v-model.number="header.diskonRp"
                                            type="number" variant="outlined" hide-details density="compact" />
                                    </v-col>
                                    <v-col cols="6">
                                        <v-text-field label="Diskon Faktur %" v-model.number="header.diskonPersen"
                                            type="number" variant="outlined" hide-details density="compact" />
                                    </v-col>
                                </v-row>
                                <v-divider class="my-4" />

                                <v-radio-group v-model="header.generate" label="Generate Otomatis" hide-details
                                    density="compact">
                                ...
                                <div v-if="header.generate === 'K'" class="pl-8 mt-2">
                                    <v-radio-group v-model="header.jenisKupon" label="Jenis Kupon" hide-details
                                        density="compact">
                                        <v-radio label="Kupon Undian" value="UNDIAN" />
                                        <v-radio label="Kupon Belanja" value="BELANJA" />
                                    </v-radio-group>
                                    <v-checkbox v-model="header.cetakKupon" true-value="Y" false-value="N"
                                        label="Cetak Kupon Otomatis" hide-details density="compact" />
                                </div>

                                <div v-if="header.generate === 'V'" class="pl-8 mt-2">
                                    <v-text-field label="Nominal Voucher Pembayaran" v-model.number="header.rpVoucher"
                                        type="number" variant="outlined" hide-details density="compact" />
                                </div>
                            </div>
                        </div>
            ...
            <v-window-item value="barang-bonus">
                <div class="form-grid-container">
                    <div class="desktop-form-section d-flex flex-column" style="min-height: 400px;">
                        ...
                        <v-data-table :headers="applicableHeaders" :items="applicableItems"
                            ...
                            <template #item.qty="{ item }"><v-text-field v-model.number="item.qty" type="number"
                            ...
                            <template #item.harga="{ item }"><v-text-field v-model.number="item.harga" type="number"
                                    variant="underlined" density="compact" hide-details class="text-end" /></template>
                            <template #item.disc="{ item }"><v-text-field v-model.number="item.disc" type="number"
                                    variant="underlined" density="compact" hide-details class="text-end" /></template>
                    ...
                    <div class="desktop-form-section d-flex flex-column" style="min-height: 400px;">
                        ...
                        <v-data-table :headers="bonusHeaders" :items="bonusItems" class="desktop-table flex-grow-1"
                            ...
                            <template #item.qty="{ item }">
                                <v-text-field v-model.number="item.qty" type="number" variant="underlined"
                                    class="text-end" density="compact" hide-details />
                            </template>
                            <template #item.actions="{ item }">
                                <v-btn v-if="item.kode" icon="mdi-delete" size="x-small" variant="text" color="error"
        ...
        <MintaBarangSearchModal v-if="isApplicableSearchVisible" source="promo-applicable"
            @close="isApplicableSearchVisible = false" @products-selected="onApplicableSelected" />
        <MintaBarangSearchModal v-if="isBonusSearchVisible" source="promo-bonus" @close="isBonusSearchVisible = false"
