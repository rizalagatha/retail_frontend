...
const variantsToUpdate = ref<any[]>([]);
...
const openUpdateModal = async (item: MasterItem) => {
    ...
    if (details.value[item.kode]) {
        ...
        try {
            ...
            toast.error(`Gagal memuat detail untuk ${item.kode}`);
        } finally {
            loading.value = false;
        }
    }
...
const calculateHpp = (variant: any) => {
    const percentage = itemToUpdate.value?.kategori === 'Rezso' ? hppPercentage.value.Rezso : hppPercentage.value.Kaosan;
    if (variant.harga > 0) {
...
const executeUpdate = async () => {
    ...
    try {
        ...
        toast.error(error.response?.data?.message || 'Gagal mengupdate harga.');
    } finally {
...
<template>
    <PageLayout title="Price List" icon="mdi-tag-multiple-outline">
        <template #header-actions>
            <v-btn v-if="authStore.can(MENU_ID, 'edit')" size="small" color="primary"
        ...
        <div class="browse-content">
            ...
            <div class="table-container">
                <v-data-table v-model="selected" :headers="headers" :items="masterData" :loading="loading"
                    ...
                    show-expand item-value="kode" @update:expanded="loadDetails">
