...
interface AllocationItem {
    ...
    ukuran: string;
    jumlah: number;
    [key: string]: any; // Untuk kolom cabang dinamis
}
...
const isEditMode = computed(() => !!route.params.nomor); // Selalu false sesuai permintaan
...
const resultItems = ref<any[]>([]); // Untuk grid 3
...
onMounted(async () => {
    ...
    try {
        ...
        allocationItems.value = data.allocationItems.map((item: any) => ({
            ...item, id: Math.random(), kdc: 0, kbs: 0, kps: 0, kpr: 0,
            k01: 0, k02: 0, k03: 0, k04: 0, k05: 0, k06: 0, total: 0
        }));
    } catch (error: any) {
        ...
        <div class="form-grid-container">
            <div class="left-column">
                <div class="desktop-form-section header-section">
                    <v-row dense>
                        <v-col cols="6"><v-text-field label="No. Terima" v-model="header.nomor" readonly filled
                        ...
                        <v-col cols="6"><v-text-field label="No. Mutasi KPS" v-model="header.nomorMutasiKps"
                                variant="outlined" density="compact" hide-details /></v-col>
                        <v-col cols="6"><v-text-field label="Tgl. Mutasi KPS" v-model="header.tanggalMutasiKps"
                                type="date" variant="outlined" density="compact" hide-details /></v-col>

                        <v-col cols="6"><v-text-field label="No. Mutasi KBS" v-model="header.nomorMutasiKbs"
                                variant="outlined" density="compact" hide-details /></v-col>
                        <v-col cols="6"><v-text-field label="Tgl. Mutasi KBS" v-model="header.tanggalMutasiKbs"
                                type="date" variant="outlined" density="compact" hide-details /></v-col>
                    </v-row>
                </div>
            ...
            <div class="right-column">
                ...
                <div class="desktop-form-section d-flex flex-column" style="height: 65%;">
                    ...
                    <v-data-table :headers="allocationHeaders" :items="allocationItems" :loading="isLoading"
                        ...
                        <template #item.kbs="{ item }"><v-text-field v-model.number="item.kbs" type="number"
                                variant="underlined" density="compact" hide-details class="text-end" /></template>
                        <template #item.kps="{ item }"><v-text-field v-model.number="item.kps" type="number"
                                variant="underlined" density="compact" hide-details class="text-end" /></template>
                        <template #item.kpr="{ item }"><v-text-field v-model.number="item.kpr" type="number"
                                variant="underlined" density="compact" hide-details class="text-end" /></template>
                        <template #item.k01="{ item }"><v-text-field v-model.number="item.k01" type="number"
                                variant="underlined" density="compact" hide-details class="text-end" /></template>
                        <template #item.k02="{ item }"><v-text-field v-model.number="item.k02" type="number"
                                variant="underlined" density="compact" hide-details class="text-end" /></template>
                        <template #item.k03="{ item }"><v-text-field v-model.number="item.k03" type="number"
                                variant="underlined" density="compact" hide-details class="text-end" /></template>
                        <template #item.k04="{ item }"><v-text-field v-model.number="item.k04" type="number"
                                variant="underlined" density="compact" hide-details class="text-end" /></template>
                        <template #item.k05="{ item }"><v-text-field v-model.number="item.k05" type="number"
                                variant="underlined" density="compact" hide-details class="text-end" /></template>
                        <template #item.k06="{ item }"><v-text-field v-model.number="item.k06" type="number"
                                variant="underlined" density="compact" hide-details class="text-end" /></template>
