...
const props = defineProps({
  ...
  customerKode: { // [BARU] Perlu customer kode untuk filter DP
    type: String,
    required: true
  }
});
  ...
  <v-dialog :model-value="true" persistent max-width="800px">
    <v-card>
      ...
      <v-card-text class="pa-0">
        <v-data-table :headers="dpTableHeaders" :items="props.dpItems" density="compact"
          class="desktop-table header-browse-blue" :items-per-page="-1" fixed-header>
          ...
          <template #[`body.append`]>
            <tr>
              <td>
                <v-text-field v-model="newDpNomor" placeholder="Cari DP (F1)..." variant="underlined" density="compact"
                  hide-details append-inner-icon="mdi-magnify" @click="openDpSearch" @keydown.f1.prevent="openDpSearch"
                  class="search-input" />
              </td>
              <td></td>
              <td></td>
    ...
    <UnpaidDpSearchModal v-if="isDpSearchVisible" :customer-kode="props.customerKode" @close="isDpSearchVisible = false"
