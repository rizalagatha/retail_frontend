...
onMounted(async () => {
    ...
    try {
        ...
        items.value = data.items.map((item: any) => ({
            ...
            jumlahTerima: item.jumlahKirim, // Default jumlah terima = jumlah kirim
        }));
    } catch (error: any) {
                    ...
                    <v-row dense>
                        ...
                        <v-col cols="12">
                            <v-text-field label="Tgl. Kirim"
                                :model-value="header.tanggalKirim ? format(parseISO(header.tanggalKirim), 'dd/MM/yyyy') : ''"
                                readonly filled density="compact" hide-details />
                        </v-col>
                        <v-col cols="12"><v-text-field label="Gudang Kirim" v-model="header.gudangAsalNama" readonly
                ...
                <div class="desktop-form-section d-flex flex-column fill-height">
                    <v-data-table :headers="tableHeaders" :items="items" :loading="isLoading" density="compact"
                        ...
                        <template #item.jumlahTerima="{ item }">
                            <v-text-field v-model.number="item.jumlahTerima" type="number" min="0"
                                :max="item.jumlahKirim" variant="underlined" density="compact" hide-details
                                class="text-end" />
                        </template>
