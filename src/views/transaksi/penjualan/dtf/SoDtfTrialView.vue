...
// [DIPERBARUI] Hapus opsi filter yang tidak perlu
const filterOptions = ref([
  ...
  { title: "No. SO Riil", value: "NoSoDtfRiil" },
  ...
  { title: "Alasan Close/Batal", value: "AlasanClose" },
  { title: "User", value: "Created" },
]);
const selectedFilterField = ref("Customer");
const filterSearchValue = ref("");

const LS_FILTER_KEY = "sodtf_trial_table_filters";
const SESSION_STATE_KEY = "sodtf_trial_browse_state";
...
// --- Header Definisi ---
// [DIPERBARUI] Hapus kolom yang tidak perlu, tambahkan No SO Riil
const headers = ref<DataTableHeader[]>([
  { title: "", key: "data-table-expand", width: 50, fixed: true },
  { title: "Nomor Trial", key: "Nomor", width: 160, fixed: true },
  { title: "Kd. Customer", key: "KdCus", width: 110 },
  { title: "Nama Customer", key: "Customer", width: 230 },
  { title: "Status", key: "status", width: 140, sortable: false },
  { title: "Revisi Ke", key: "RevisiKe", align: "center", width: 100 },
  ...
  { title: "No. SO Riil", key: "NoSoDtfRiil", width: 160 },
  { title: "Sales", key: "Sales", width: 140 },
  { title: "Bag. Desain", key: "BagDesain", width: 140 },
  { title: "Kain", key: "Kain", width: 130 },
  { title: "Finishing", key: "Finishing", width: 130 },
  { title: "Workshop", key: "Workshop", width: 130 },
  { title: "Keterangan", key: "Keterangan", width: 250 },
  { title: "Alasan Batal", key: "AlasanClose", width: 200 },
  { title: "User", key: "Created", width: 120 },
  { title: "User Modified", key: "UserModified", width: 130 },
...
const formatFilterValue = (key: string, val: string | number | null | undefined): string => {
  if (["Tanggal", "DateModified"].includes(key)) {
    if (!val) return "-";
    try {
...
const resetAllFilters = () => {
  ...
  if (route.query.status || route.query.startDate) router.replace({ query: {} });
};

const isFilterActive = (key: string): boolean => Boolean(columnFilters.value[key]);
const filterType = (key: string): string => columnFilters.value[key]?.type || "";
...
const fetchCabangList = async () => {
  try {
    ...
    if (userCabang === "K06") {
      cabangList.value = [{ kode: "ALL", nama: "SEMUA CABANG LUAR" }, ...response.data];
...
const fetchData = async () => {
  ...
  try {
    const response = await api.get("/so-dtf-trial", { params: filters });
    soDtfList.value = response.data;
  } catch {
    toast.error("Gagal memuat data SO DTF Trial.");
  } finally {
...
const loadDetails = async (newlyExpandedItems: SoDtfHeader[]) => {
  ...
  try {
    const response = await api.get(`/so-dtf-trial/${nomor}`);
...
const getRowTextColor = (item: SoDtfHeader) => {
  ...
  if (!item.NoSoDtfRiil) return "text-red font-weight-bold";
...
const submitCloseSo = async () => {
  ...
  try {
    await api.post("/so-dtf-trial/close", {
      nomor: itemToClose.value.Nomor,
      alasan: closeReason.value,
      user: authStore.user?.kode,
    });
    toast.success("SO DTF Trial berhasil dibatalkan.");
    isCloseDialogVisible.value = false;
    fetchData();
    selected.value = [];
  } catch {
    toast.error("Gagal menutup SO DTF Trial.");
  }
};
...
const handleEdit = () => {
  ...
  const isReadOnly = selectedItem.NoSoDtfRiil !== "" || selectedItem.Close === "Y";
  router.push({
    path: `/transaksi/penjualan/dtf/so-dtf-trial/ubah/${selectedItem.Nomor}`,
...
const executeDelete = async () => {
  ...
  try {
    await api.delete(`/so-dtf-trial/${itemToDelete.value.Nomor}`);
    toast.success(`SO DTF Trial ${itemToDelete.value.Nomor} berhasil dihapus.`);
    fetchData();
    selected.value = [];
  } catch (error: unknown) {
  ...
  try {
    ...
    if (type === "header") {
      const dataToExport = targetData.map((item) => ({
        "Nomor Trial": item.Nomor,
        "Status Transaksi": item.status,
        "Revisi Ke": item.RevisiKe === 0 ? "Desain Awal" : item.RevisiKe,
        ...
        "Alasan Batal": item.AlasanClose,
        User: item.Created,
      }));

      const worksheet = XLSX.utils.json_to_sheet(dataToExport);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "SO DTF Trial Header");
      XLSX.writeFile(workbook, "Export_SO_DTF_Trial_Header.xlsx");
      ...
      const response = await api.post("/so-dtf-trial/export-detail", { nomors: daftarNomor });
      ...
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "SO DTF Trial Detail");
      XLSX.writeFile(workbook, "Export_SO_DTF_Trial_Detail.xlsx");
  ...
  if (hasViewPermission.value) {
    ...
    if (savedState) {
      try {
        ...
        if (parsedState.filters) Object.assign(filters, parsedState.filters);
        ...
        console.error("Gagal membaca state filter", e);
      }
    } else {
...
watch(
  ...
  () => {
    if (isMounted.value) {
      ...
      if (fetchTimeout.value) clearTimeout(fetchTimeout.value);
      fetchTimeout.value = window.setTimeout(() => fetchData(), 300);
    }
  },
  { deep: true }
);

watch([filterSearchValue, selectedFilterField], saveStateToSession);
watch(columnFilters, (val) => localStorage.setItem(LS_FILTER_KEY, JSON.stringify(val)), {
  deep: true,
});

onBeforeRouteLeave((to, from, next) => {
  if (!to.path.includes("/so-dtf-trial")) {
    sessionStorage.removeItem(SESSION_STATE_KEY);
  }
  next();
});
...
<template>
  <PageLayout title="SO DTF Trial Pesanan" desktop-mode icon="mdi-flask-outline">
    ...
    <div v-else class="browse-content">
      <div class="filter-section">
        ...
        >
          Status: {{ filters.status === "belum_so" ? "Belum SO Riil" : filters.status }}
        </v-chip>
        <v-spacer></v-spacer>
        <v-btn
          color="error"
      ...
      <div class="legend-section">
        <div class="legend-group">
          <strong class="legend-title">Status Trial:</strong>
          <div class="legend-item">
            <span class="text-blue font-weight-medium">Teks Biru</span>: Sudah Jadi SO Riil (Closed)
          </div>
          <div class="legend-item">
            <span class="text-red font-weight-medium">Teks Merah</span>: Masih Trial (Open)
          </div>
          <div class="legend-item">
            <span class="row-color-sample-closed"></span> Dibatalkan / Close Manual
          </div>
        </div>
      </div>
      ...
      <div class="table-container">
        <AppDataTable
          ...
          :item-props="(item: SoDtfHeader) => ({ class: item.Close === 'Y' || item.AlasanClose ? 'row-closed' : '' })"
        >
          ...
          >
            <td :class="getRowTextColor(item)">
              <template v-if="['Tanggal', 'DateModified'].includes(header.key)">
                {{ formatDate(item[header.key]) }}
              </template>
              <template v-else-if="header.key === 'status'">
                <v-chip
                  v-if="item.status === 'Closed (Jadi SO)'"
                  color="success"
                  variant="tonal"
                  size="x-small"
                  >Telah Menjadi SO Riil</v-chip
                >
                <v-chip
                  v-else-if="item.status === 'Closed (Batal)' || item.status === 'Closed'"
                  color="blue-grey"
                  variant="tonal"
                  size="x-small"
                  >Dibatalkan</v-chip
                ...
                <v-chip v-else color="grey" variant="tonal" size="x-small"
                  >Masih Trial (Open)</v-chip
                >
              </template>

              <template v-else-if="header.key === 'RevisiKe'">
                <v-chip
                  size="x-small"
                  :color="item.RevisiKe > 0 ? 'orange-darken-3' : 'grey-lighten-1'"
                  label
                >
                  {{ item.RevisiKe === 0 ? "Desain Awal" : `Rev ${item.RevisiKe}` }}
                </v-chip>
              </template>
    ...
    <v-dialog v-model="isCloseDialogVisible" max-width="500px" persistent>
      <v-card>
        <v-card-title class="text-h6">Isi Alasan Membatalkan Trial</v-card-title>
        <v-card-text>
          <p class="text-caption mb-2">
            Anda akan membatalkan Trial Nomor: <strong>{{ itemToClose?.Nomor }}</strong>
          </p>
          <v-textarea
            v-model="closeReason"
            label="Alasan Batal / Close"
            rows="3"
            variant="outlined"
            autofocus
          ></v-textarea>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn text @click="isCloseDialogVisible = false">Tutup</v-btn>
          <v-btn color="primary" @click="submitCloseSo">Simpan Batal</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
