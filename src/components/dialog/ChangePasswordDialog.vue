...
const { showPasswordDialog, closePasswordDialog } = usePasswordDialog();
...
const handleChangePassword = async () => {
  ...
  try {
    ...
    resetForm();
    closePasswordDialog(); // Tutup dialog setelah sukses

  } catch (error: unknown) {
    const axiosError = error as AxiosError<{ message?: string }>;
    const message = axiosError.response?.data?.message || 'Terjadi kesalahan saat mengubah password';
    toast.error(message);
  } finally {
  ...
  <v-dialog v-model="showPasswordDialog" max-width="500px" persistent>
    <v-card>
      <v-card-title class="d-flex align-center bg-grey-lighten-4 pa-4">
        ...
        <span class="text-h6">Ganti Password</span>
        <v-spacer></v-spacer>
        <v-btn icon="mdi-close" variant="text" size="small" @click="handleClose" :disabled="isLoading"></v-btn>
      </v-card-title>
      ...
      <v-card-actions class="pa-4">
        <v-btn variant="text" @click="handleClose" :disabled="isLoading">Batal</v-btn>
        <v-spacer></v-spacer>
        <v-btn color="primary" variant="elevated" :loading="isLoading" :disabled="!isFormValid || isLoading"
