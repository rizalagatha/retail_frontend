...
<template>
    <PageLayout :title="isEditMode ? 'Ubah Stok Opname' : 'Buat Stok Opname'" :menu-id="MENU_ID">
        <template #header-actions>
            <v-btn size="small" color="primary" @click="handleSave" :loading="isLoading"
                :disabled="isReadOnly">Simpan</v-btn>
        ...
        <div class="form-grid-container">
            <div class="left-column">
                <div class="desktop-form-section header-section">
                    <v-row dense>
                        <v-col cols="6"><v-text-field label="SOP" v-model="formHeader.nomor" readonly filled
                                density="compact" hide-details /></v-col>
                        <v-col cols="6"><v-text-field label="Tanggal" v-model="formHeader.tanggal" type="date" readonly
                        ...
                        <v-col cols="6">
                            <v-text-field v-model="formHeader.gudangNama" label="Nama Gudang" readonly filled
                                density="compact" hide-details />
                        </v-col>
                        <v-col cols="12">
                            <v-text-field label="Keterangan" v-model="formHeader.keterangan" density="compact"
                                :readonly="isReadOnly" hide-details>
                            </v-text-field>
                        </v-col>
                        ...
                        <v-btn color="info" @click="handleHitungStok" :loading="isLoading"
                            :disabled="isReadOnly || isEditMode" prepend-icon="mdi-calculator">
                            Hitung Stok
                        </v-btn>
                    </v-row>
                </div>
                    ...
                    <v-data-table :headers="headers" :items="items" :loading="isLoading"
                        ...
                        <template v-slot:[`item.Kode`]="{ item, index }">
                            <v-text-field v-model="item.Kode" variant="underlined" density="compact" hide-details
                                :readonly="!!item.Nama" placeholder="Scan Barcode..."
                                @keydown.enter.prevent="handleItemAdd(index)" />
                        </template>

                        <template v-slot:[`item.Jumlah`]="{ item }">
                            <v-text-field :id="`jumlah-${item.id}`" v-model.number="item.Jumlah" type="number"
