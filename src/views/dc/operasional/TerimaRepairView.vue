...
// --- Konfigurasi Tabel ---
const headers = [
  { title: "Nomor Kirim", key: "nomor" },
  { title: "Tanggal Kirim", key: "tanggal" },
  { title: "Nomor Tolak STBJ", key: "nomorTolak" },
  { title: "Nomor STBJ", key: "stbj" },
  { title: "Nomor Terima", key: "nomorTerima" },
  { title: "Tgl Terima", key: "tglTerima" },
  { title: "Dari Gudang", key: "namaGudang" },
  { title: "User", key: "user_create" },
];
const detailHeaders = [
  { title: "SPK", key: "spk" },
  { title: "Kode", key: "kode" },
...
const handleTerima = () => {
  const row = selectedRow.value; // [PERBAIKAN] Tangkap ke variabel lokal
  if (!canTerima.value || !row) return;

  router.push({ name: "TerimaRepairCreate", query: { nomorKirim: row.nomor } });
};
...
const handleBatalTerima = () => {
  ...
  showConfirmation(
    ...
    `Yakin membatalkan penerimaan untuk <strong>${row.nomor}</strong>?`,
    async () => {
      try {
        const response = await api.delete(`/terima-repair/cancel-receipt/${row.nomor}`);
        toast.success(response.data.message);
        fetchMasterData();
      } catch (error: unknown) {
        // [PERBAIKAN] Amankan catch block
