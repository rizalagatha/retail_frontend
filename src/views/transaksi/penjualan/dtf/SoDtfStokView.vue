...
<template>
  <PageLayout title="SO DTF Stok" desktop-mode icon="mdi-package-variant-closed">
    <template #header-actions>
      ...
      <v-btn size="small" :disabled="!isSingleSelected" color="orange-darken-2" @click="openCloseDialog">
        Close SO
      </v-btn>
    </template>
    ...
    <div v-else class="browse-content">
      <div class="filter-section">
        ...
        <v-text-field v-model="endDate" type="date" density="compact" hide-details variant="outlined"
          style="min-width: 140px;" />
        <v-select v-model="selectedCabang" :items="cabangList" item-title="nama" item-value="kode" label="Cabang"
          density="compact" hide-details variant="outlined" class="ms-2" style="min-width: 180px;" />
        <v-divider vertical class="mx-2"></v-divider>
          ...
          <template v-for="header in headers.filter(h => h.key !== 'data-table-expand')"
            ...
            <td>
              <template v-if="['Tanggal', 'TglPengerjaan', 'Created'].includes(header.key)">
                {{ item[header.key] ? format(parseISO(item[header.key] as string), header.key === 'Created' ?
