  ...
  try {
    ...
    // --- 2. FALLBACK: PROMO DESEMBER/JANUARI (Jika Promo Februari tidak ada) ---
      ...
      const promoReguler = promos.find(
        (p) => p.pro_nomor === "PRO-2025-010" || p.pro_judul.toUpperCase().includes("REGULER")
      );

      if (promoReguler) {
        promoMessages.push(
          `🔥 PROMO BULANAN: Potongan Rp 25.000 tiap kelipatan Rp 250.000 (Khusus Kaos Polos/Reguler, Non-Jersey).`
        );
      }
    }
