...
const userCabangNama = computed(
    () =>
        (authStore.user as { cabangNama?: string; CabangNama?: string })
            ?.cabangNama ||
        (authStore.user as { cabangNama?: string; CabangNama?: string })
            ?.CabangNama ||
        "",
...
// Status Kuncian Edit: Dokumen HANYA terkunci (tidak bisa diedit) jika sudah ada TTD Penerima
const hasTtdPenerima = computed(() =>
    Boolean(header.ttdDriver && String(header.ttdDriver).trim() !== ""),
);
const isDocumentLocked = computed(
    () => !isNew.value && hasTtdPenerima.value,
);
const isEkspedisi = computed(
    () => (header.jenisKirim || "").toUpperCase() === "EKSPEDISI",
);

// 1. Ekspedisi: hanya boleh input resi saja jika dokumen terkunci
const isResiOnlyEdit = computed(
    () => isDocumentLocked.value && isEkspedisi.value,
);
...
const initCanvas = () => {
    ...
    const existing =
        ttdRole.value === "pengirim" ? header.ttdPengirim : header.ttdDriver;
    if (existing) {
        const img = new Image();
        img.onload = () => {
...
// Modal Selection Available SJ / Invoice KPR & Barcode Scanner
const showSjModal = ref(false);
const docTypeModal = ref<"SJ" | "KPR">("SJ");
const availableSjList = ref<AvailableSjItem[]>([]);
const availableKprList = ref<AvailableSjItem[]>([]);
const loadingAvailableSj = ref(false);
const loadingAvailableKpr = ref(false);
const sjSearch = ref("");
const kprSearch = ref("");
const scannedBarcode = ref("");
const barcodeInputRef = ref<{ focus: () => void; $el?: HTMLElement } | null>(
    null,
);
    ...
    setTimeout(() => {
        if (barcodeInputRef.value) {
            if (typeof barcodeInputRef.value.focus === "function") {
                ...
                (
                    barcodeInputRef.value.$el.querySelector(
                        "input",
                    ) as HTMLInputElement
                )?.focus();
            }
...
const onGudangAsalSelected = (selectedGudang: {
    kode: string;
    nama: string;
}) => {
...
const onGudangTujuanSelected = (selectedGudang: {
    kode: string;
    nama: string;
}) => {
    if (items.value.length > 0) {
        toast.warning(
            "Tujuan Pengiriman terkunci karena daftar muatan sudah terisi.",
        );
...
const onGudangAsalSearch = (val: string) => {
    if (!val) return;
    const match = cabangList.value.find(
        (c) => c.kode.toUpperCase() === val.trim().toUpperCase(),
    );
    if (match) {
...
const displayManifestNomor = computed(() => {
    ...
    const gudangKode = header.gudang
        ? header.gudang.trim().toUpperCase()
        : "KDC";
...
const getCabangNama = (kode?: string): string => {
    if (!kode) return "";
    const match = cabangList.value.find(
        (c) => c.kode.toUpperCase() === kode.trim().toUpperCase(),
    );
...
// Filter Available Dokumen (SJ atau Invoice KPR) yang belum ada di daftar items saat ini & sesuai header.tujuan
const filteredAvailableSj = computed(() => {
    const isKpr = docTypeModal.value === "KPR";
    const sourceList = isKpr ? availableKprList.value : availableSjList.value;
    const existingDocSet = new Set(
        items.value
            .filter((i) => i.sjNomor)
            .map((i) => (i.sjNomor as string).toUpperCase()),
    );
    return sourceList.filter((doc) => {
        // 1. Abaikan Dokumen yang sudah dimuat di tabel
        if (existingDocSet.has(doc.sjNomor.toUpperCase())) return false;

        // 2. Jika tujuan pengiriman di header sudah dipilih/terisi,
        // filter hanya Dokumen yang menuju ke store/customer tujuan yang sama!
        if (header.tujuan) {
            return (
                doc.storeKode.toUpperCase() ===
                header.tujuan.trim().toUpperCase()
            );
        }
...
// Total Computation
const totalSj = computed(
    () => items.value.filter((i) => !i.isCustom && i.sjNomor).length,
);
...
const handlePrint = () => {
    ...
    if (!nomor) {
        toast.error(
            "Simpan dokumen manifest terlebih dahulu sebelum mencetak.",
        );
        return;
    }
...
const otherItemOptions = (currentIndex: number) => {
    return items.value
        ...
        .filter(
            ({ item, idx }) => idx !== currentIndex && Number(item.koli) > 0,
        )
        .map(({ item, idx }) => {
            const label =
                item.sjNomor || item.namaBarang || `Baris #${idx + 1}`;
            return {
                title: label,
                value: label,
            };
...
const loadAvailableSj = async () => {
    ...
    try {
        const response = await api.get<AvailableSjItem[]>(
            "/manifest-kirim/available-sj",
            {
                params: {
        ...
        toast.error(
            err.response?.data?.message || "Gagal memuat daftar Surat Jalan.",
        );
    } finally {
...
// Load Available Invoice KPR via api instance
const loadAvailableKpr = async () => {
    loadingAvailableKpr.value = true;
    try {
        const response = await api.get<AvailableSjItem[]>(
            "/manifest-kirim/available-inv-kpr",
            {
                params: {
                    gudang: header.gudang,
                    search: kprSearch.value || sjSearch.value,
                },
            },
        );
        availableKprList.value = response.data;
    } catch (error: unknown) {
        const err = error as { response?: { data?: { message?: string } } };
        toast.error(
            err.response?.data?.message || "Gagal memuat daftar Invoice KPR.",
        );
    } finally {
        loadingAvailableKpr.value = false;
    }
};
...
const openKprModal = () => {
    docTypeModal.value = "KPR";
    kprSearch.value = "";
    showSjModal.value = true;
    loadAvailableKpr();
};

// Tambah Dokumen (SJ / Invoice KPR) ke Manifest
const addSjToManifest = (sj: AvailableSjItem) => {
    const isKpr =
        sj.tipeDokumen === "Invoice KPR" ||
        sj.sjNomor.toUpperCase().startsWith("KPR.INV");
    const docLabel = isKpr ? "Invoice KPR" : "Surat Jalan";

    // 1. Cek jika header.tujuan sudah diisi dan beda dengan storeKode dokumen yang coba ditambahkan
    if (
        header.tujuan &&
        header.tujuan.trim().toUpperCase() !== sj.storeKode.trim().toUpperCase()
    ) {
        toast.error(
            `${docLabel} ${sj.sjNomor} bertujuan ke "${
    ...
    // 2. Cek jika di tabel sudah ada dokumen lain yang tujuannya berbeda
    const existingSj = items.value.find((i) => !i.isCustom && i.storeKode);
    if (
        existingSj &&
        existingSj.storeKode &&
        existingSj.storeKode.trim().toUpperCase() !==
            sj.storeKode.trim().toUpperCase()
    ) {
        toast.error(
            `${docLabel} ${sj.sjNomor} bertujuan ke "${
                sj.storeNama || sj.storeKode
                ...
                sj.storeKode
            }), berbeda dengan dokumen sebelumnya yang menuju ke "${
                existingSj.storeNama || existingSj.storeKode
            }" (${
                existingSj.storeKode
            }). Semua dokumen dalam satu manifest harus menuju ke tujuan yang sama.`,
        );
        return;
    }
    ...
    // Pastikan dokumen selalu berada di atas kelompok Barang Lain-lain
    ...
    // Jika di atasnya ada dokumen, samakan tujuan seluruh barang lain-lain mengikuti dokumen ini
    items.value.forEach((it) => {
        if (it.isCustom) {
    ...
    // Jika header.tujuan belum diisi, otomatis isi dari dokumen pertama ini!
    if (!header.tujuan) {
        header.tujuan = sj.storeKode;
    }

    toast.success(`${docLabel} ${sj.sjNomor} ditambahkan ke manifest.`);
    focusBarcodeInput();
};
...
const processSingleCode = async (code: string) => {
    ...
    const exists = items.value.some(
        (i) =>
            ...
            (i.noPackingList &&
                i.noPackingList.toUpperCase() === code.toUpperCase()),
    );
    if (exists) {
    ...
    // Khusus barcode Invoice KPR (berawalan KPR.INV)
    if (code.toUpperCase().startsWith("KPR.INV")) {
        const match = availableKprList.value.find(
            (inv) =>
                inv.sjNomor.toUpperCase() === code.toUpperCase() ||
                (inv.noRefSj &&
                    inv.noRefSj.toUpperCase() === code.toUpperCase()),
        );
        ...
        try {
            ...
            if (found) {
                ...
                availableKprList.value = response.data;
                kprSearch.value = code;
                docTypeModal.value = "KPR";
                showSjModal.value = true;
                toast.info(
                    `Ditemukan ${response.data.length} Invoice KPR. Silakan pilih.`,
                );
            } else {
                toast.error(
                    `Invoice KPR "${code}" tidak ditemukan atau sudah dimanifestasikan.`,
                );
            }
        } catch (error: unknown) {
            const err = error as { response?: { data?: { message?: string } } };
            toast.error(
                err.response?.data?.message || "Gagal memindai Invoice KPR.",
            );
        }
        return;
    }
    ...
    const match = availableSjList.value.find(
        (sj) =>
            ...
            (sj.noPackingList &&
                sj.noPackingList.toUpperCase() === code.toUpperCase()),
    );
    ...
    try {
        ...
        if (found) {
            ...
            showSjModal.value = true;
            toast.info(
                `Ditemukan ${response.data.length} Surat Jalan. Silakan pilih.`,
            );
        } else {
            toast.error(`No. Dokumen "${code}" tidak ditemukan.`);
        }
    } catch (error: unknown) {
        const err = error as { response?: { data?: { message?: string } } };
        toast.error(
            err.response?.data?.message || "Gagal memindai dokumen pengiriman.",
        );
    }
};
...
const loadEditData = async () => {
    ...
    try {
        ...
        header.gudang = String(
            h.gudang || h.Gudang || userCabangKode.value || "",
        );
        ...
        const allReceivedItems = (response.data.items ||
            []) as (ManifestKirimItem & {
            namaBarang?: string;
        })[];
        ...
        items.value = allReceivedItems
            .map((it) => {
                const isCustom =
                    !it.sjNomor || String(it.sjNomor).trim() === "";
                return {
                    ...it,
                    isCustom,
                    sjNomor: it.sjNomor || "",
        ...
        const err = error as { response?: { data?: { message?: string } } };
        toast.error(
            err.response?.data?.message || "Gagal memuat data edit manifest.",
        );
...
const submitForm = async () => {
    ...
    if (items.value.length === 0) {
        toast.error(
            "Tambahkan minimal satu Surat Jalan atau Barang Lain-lain ke dalam manifest.",
        );
        return;
    }
    ...
    if (firstSj) {
        ...
        if (
            header.tujuan &&
            header.tujuan.trim().toUpperCase() !== primaryStore
        ) {
            toast.error(
    ...
    for (let idx = 0; idx < items.value.length; idx++) {
        ...
        if (item.isCustom) {
            if (!item.namaBarang || item.namaBarang.trim() === "") {
                toast.error(
                    `Nama / Deskripsi Barang pada baris ke-${
                        idx + 1
                    } wajib diisi.`,
                );
                return;
            ...
            if (!item.sjNomor) {
                toast.error(
                    `Nomor Surat Jalan pada baris ke-${idx + 1} tidak valid.`,
                );
                return;
            }
        }
        ...
        if (
            ...
            toast.error(
                `Jumlah Koli pada baris ke-${
                    idx + 1
                } tidak valid (harus angka, minimal 0).`,
            );
            return;
        }
        ...
        if (Number(item.koli) === 0 && !item.referensiGabung) {
            const itemLabel =
                item.sjNomor || item.namaBarang || `Baris #${idx + 1}`;
            toast.error(
                `Item "${itemLabel}" memiliki Koli 0. WAJIB pilih referensi penggabungan!`,
            );
            return;
        }
    }
    ...
    try {
        const response = await api.post<{ message: string; nomor: string }>(
            "/manifest-kirim",
            {
                header: {
                    ...header,
                    beratKg: Number(header.beratKg) || 0,
                },
                items: payloadItems,
                isNew: isNew.value,
            },
        ...
        const err = error as { response?: { data?: { message?: string } } };
        toast.error(
            err.response?.data?.message || "Gagal menyimpan Manifest Kirim.",
        );
    } finally {
        saving.value = false;
    }
...
<template>
    ...
    >
        <template #header-actions>
            ...
            <v-btn size="small" prepend-icon="mdi-refresh" @click="goBack">
                Batal
            </v-btn>
            <v-btn size="small" prepend-icon="mdi-close" @click="goBack">
                Tutup
            </v-btn>
        </template>
        ...
        >
            Manifest ini sudah ditandatangani oleh Penerima. Seluruh data telah dikunci dan tidak dapat diubah.
        </v-alert>
        <v-alert
            ...
            class="mb-3 font-weight-medium"
        >
            Manifest pengiriman ekspedisi ini sudah ditandatangani oleh Penerima.
            Perubahan hanya diperbolehkan untuk <strong>No. Resi</strong> saja.
        </v-alert>

        <div
            class="form-grid-container"
            :class="{ 'hide-left': !isLeftColumnVisible }"
        >
            <!-- Left Column: Header -->
                    ...
                    <v-row dense>
                        ...
                        <v-col cols="12">
                            <v-select
                                ...
                                :items="[
                                    {
                                        title: 'Internal (Armada Sendiri)',
                                        value: 'ARMADA_SENDIRI',
                                    },
                                    {
                                        title: 'Ekspedisi / Paket',
                                        value: 'EKSPEDISI',
                                    },
                                    {
                                        title: 'Ambil Sendiri',
                        ...
                        <v-col
                            cols="12"
                            v-if="header.jenisKirim === 'EKSPEDISI'"
                        >
                            <v-combobox
                                label="Ekspedisi"
                        ...
                        >
                            <v-text-field
                                ...
                                @keydown.enter.prevent="
                                    header.noResi = header.noResi?.trim() ?? ''
                                "
                            >
                                <template #append-inner>
                                    <v-tooltip
                                        location="top"
                                        text="Fokus ke scanner untuk scan barcode resi"
                                    >
                    ...
                    >
                        ...
                        <tbody>
                            <tr v-if="items.length === 0">
                                ...
                                >
                                    Belum ada Surat Jalan, Invoice KPR, atau
                                    Barang Lain-lain dalam manifest ini. Klik
                                    <strong>"+ Tambah SJ"</strong>,
                                    <strong>"+ Tambah Invoice KPR"</strong>,
                                    atau
                                    <strong>"+ Tambah Barang Lain-lain"</strong
                                    >.
                                </td>
                            </tr>
                            <template
                            ...
                            >
                                ...
                                >
                                    <td
                                        colspan="11"
                                        class="py-1 px-3 text-blue-grey-darken-4 border-t border-b"
                                    >
                                ...
                                <tr class="table-row-hover">
                                    ...
                                    <!-- Jika Item Dokumen (SJ / Invoice KPR): 3 kolom terpisah (No Dokumen, Tanggal, No PL/Ref) -->
                                    <template v-if="!item.isCustom">
                                        <td
                                            class="font-weight-bold"
                                            :class="
                                                item.sjNomor?.startsWith(
                                                    'KPR.INV',
                                                )
                                                    ? 'text-deep-purple'
                                                    : 'text-primary'
                        ...
                        <tfoot v-if="items.length > 0">
                            <tr
                                class="bg-grey-lighten-4 font-weight-bold text-caption"
                            >
                                <td
                                    colspan="5"
                                    class="text-end font-weight-bold uppercase pe-4"
                                >
                                    TOTAL :
                                </td>
                                <td
                                    class="text-end text-primary font-weight-bold"
                                >
                                    {{ totalQtyPcs }}
                                </td>
                                <td
                                    class="text-end text-success font-weight-bold"
                                >
                                    {{ totalKoliFisik }}
                                </td>
                                <td colspan="4"></td>
        ...
        >
                ...
                >
                    <div>
                        ...
                        <v-card-text class="pa-3 text-caption">
                            ...
                            >
                                <span class="text-grey-darken-1"
                                    >Barang Lain-lain</span
                                >
                                <span class="font-weight-bold text-primary"
                                    >{{ totalCustom }} Item</span
                                >
                            </div>
                            <div class="d-flex justify-space-between mb-1">
                                <span class="text-grey-darken-1"
                                ...
                                >
                                <span class="font-weight-bold text-primary"
                                    >{{ totalQtyPcs }} Pcs</span
                                >
                            </div>
                            <div
                                class="d-flex justify-space-between mb-1"
                                v-if="Number(header.beratKg) > 0"
                            >
                                <span class="text-grey-darken-1">Total Berat</span>
                                <span class="font-weight-bold text-primary"
                                    >{{ header.beratKg }} Kg</span
                                >
                            </div>
                        </v-card-text>
                    </div>
                    ...
                    <div class="pa-3 border-t text-center">
                        ...
                        >
                            {{
                                header.ttdDriver
                                    ? isFieldLocked
                                        ? "✓ TTD Penerima (Terkunci)"
                                        : "✓ Lihat / Ubah TTD Penerima"
                                    : "TTD Penerima (Driver)"
                            }}
                        </v-btn>
                    </div>
            ...
            <v-col cols="12" sm="6" md="3">
                <v-card
                    class="rounded-lg border elevation-1 h-100 d-flex flex-column"
                >
                    <v-card-title
                    ...
                    >
                        ...
                        <div
                            class="qr-box pa-2 border rounded-lg bg-white mb-2 elevation-1 d-inline-block"
                        >
                            <QrcodeVue
                        ...
                        <div
                            class="font-weight-bold text-caption text-primary mb-2"
                        >
                        ...
                        <div
                            class="stepper-vertical text-start w-100 border-t pt-2 mt-1"
                        >
                            <!-- Step 1: Pembuatan / Tercatat -->
                            <div
                                class="step-item d-flex align-center gap-2 mb-2"
                            >
                                <div
                                    :class="[
                                        'step-circle',
                                ...
                                >
                                    <v-icon size="12" color="white">{{
                                        isNew ? "mdi-pencil" : "mdi-check"
                                    }}</v-icon>
                                </div>
                                <div>
                                    <div
                                        class="font-weight-bold text-caption text-grey-darken-3"
                                    >
                                        1.
                                        {{
                                            isNew
        ...
        <!-- Dialog Lookup Pilih Surat Jalan / Invoice KPR yang Tersedia -->
        <v-dialog v-model="showSjModal" max-width="900px">
            <v-card class="rounded-lg">
                <v-card-title
                    :class="[
                        docTypeModal === 'KPR'
                            ...
                            : 'bg-primary',
                        'text-white d-flex align-center justify-space-between py-3 px-4',
                    ]"
                >
                    <div class="d-flex align-center gap-2">
                        <span class="text-subtitle-1 font-weight-bold">
                            {{
                                docTypeModal === "KPR"
                                    ? "Pilih Invoice untuk Dimuat"
                                    : "Pilih Surat Jalan untuk Dimuat"
                            }}
                        </span>
                        <v-chip
                            v-if="header.tujuan"
                            color="white"
                            class="bg-white text-dark align-center font-weight-bold ms-2"
                            size="small"
                        >
                ...
                <v-card-text class="pt-4">
                    <!-- Tab selector antara Surat Jalan dan Invoice KPR -->
                    <div class="d-flex gap-2 mb-3">
                        <v-btn
                            size="small"
                            :variant="
                                docTypeModal === 'SJ' ? 'flat' : 'outlined'
                            "
                            color="primary"
                        ...
                        <v-btn
                            size="small"
                            :variant="
                                docTypeModal === 'KPR' ? 'flat' : 'outlined'
                            "
                            color="deep-purple-darken-1"
                    ...
                    <v-row density="compact" class="mb-3">
                        <v-col cols="12" sm="8">
                            ...
                            <v-text-field
                                v-else
                                v-model="kprSearch"
                                label="Cari No. Invoice / Customer / Ref SJ"
                                prepend-inner-icon="mdi-magnify"
                                density="compact"
                                hide-details
                                clearable
                                variant="outlined"
                                @keyup.enter="loadAvailableKpr"
                            />
                        </v-col>
                        <v-col cols="12" sm="4">
                            <v-btn
                        ...
                        <tbody>
                            ...
                            <tr v-else-if="filteredAvailableSj.length === 0">
                                ...
                                >
                                    ...
                                    yang tersedia/siap dikirim.
                                </td>
                            </tr>
                            <tr
                                v-for="doc in filteredAvailableSj"
                                :key="doc.sjNomor"
                            >
                                <td
                                    class="font-weight-medium"
                                ...
                                <td>
                                    {{ doc.storeNama }} ({{ doc.storeKode }})
                                </td>
                                <td v-if="docTypeModal === 'KPR'">
                                    {{ doc.noMinta || doc.noRefSj || "-" }}
                                </td>
                                <td
            ...
            <v-card class="rounded-lg">
                ...
                <v-card-text class="pa-4 text-center">
                    <p class="text-caption text-grey-darken-1 mb-2">
                        Silakan goreskan tanda tangan pada papan di bawah ini
                        menggunakan Mouse atau Layar Sentuh.
                    </p>
                ...
                <v-card-actions
                    class="pa-3 border-t bg-grey-lighten-5 d-flex justify-space-between"
                >
                    <v-btn
                        color="error"
                        variant="outlined"
