...
const numCol = (title: string, key: string, width: number) => ({
  title, key, width, align: 'end' as const, sortable: true
});
...
const activeHeaders = computed(() => {
  switch (reportType.value) {
    case 'tanggal': return headersTanggal.value;
    case 'customer': return headersCustomer.value;
    case 'level': return headersLevel.value;
    default: return [];
  }
});
    ...
    <div v-else class="browse-content">
      <div class="filter-section">
        ...
        <v-select v-model="filters.gudangKode" :items="cabangList" item-title="nama" item-value="kode" label="Cabang"
          density="compact" hide-details variant="outlined" style="max-width: 200px;" class="ms-4"
      ...
      <div class="table-container">
        <AppDataTable :headers="activeHeaders" :items="masterData" :loading="isLoading"
          class="desktop-table header-browse-blue" density="compact" fixed-header :show-expand="reportType === 'level'"
          item-value="Kode" v-model:expanded="expanded" @update:expanded="loadDetails" :items-per-page="-1">
            ...
            <tr>
              <template v-for="header in columns" :key="header.key">
                <th
                  ...
                  :class="{ 'text-center': header.align === 'center', 'text-end': header.align === 'start' }"
          ...
          <template #[`item.data-table-expand`]="{ internalItem, toggleExpand, isExpanded }">
            <v-btn v-if="reportType === 'level'" icon="mdi-chevron-down"
              :class="{ 'rotate-180': isExpanded(internalItem) }" size="x-small" variant="text"
              @click.stop="toggleExpand(internalItem)" />
          </template>

          <template v-for="header in activeHeaders.filter(h => h.key !== 'data-table-expand')"
            #[`item.${header.key}`]="{ item }" :key="header.key">
            <td>
              <template v-if="['Nominal', 'Hpp', 'Laba', 'Donasi', 'PundiAmal'].includes(header.key)">
              ...
              <td :colspan="columns.length" class="pa-0">
                <div class="detail-container">
                  <div class="detail-table-wrapper">
                    <div v-if="loadingDetails.has(item.Kode)" class="text-center pa-4 text-caption">Memuat detail...
                    </div>
                    <v-data-table v-else :headers="detailHeadersLevel" :items="details[item.Kode] || []"
                      density="compact" class="detail-table" :items-per-page="-1" hide-default-footer>
                      <template #[`item.Nominal`]="{ item: dItem }">{{ formatRupiah(dItem.Nominal) }}</template>
                      <template #[`item.Hpp`]="{ item: dItem }">{{ formatRupiah(dItem.Hpp) }}</template>
                      <template #[`item.Laba`]="{ item: dItem }">{{ formatRupiah(dItem.Laba) }}</template>
                      <template #bottom></template>
                    </v-data-table>

                    <div
                      v-if="!loadingDetails.has(item.Kode) && (!details[item.Kode] || details[item.Kode].length === 0)"
