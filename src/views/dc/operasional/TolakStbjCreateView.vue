...
const save = () => {
  ...
  showConfirmation(
    "Konfirmasi Simpan",
    "Anda yakin ingin menyimpan data penolakan ini?",
    executeSave
  );
};
...
onMounted(async () => {
  ...
  try {
    const response = await api.get(`/tolak-stbj-form/load-from-stbj`, {
  ...
  <PageLayout title="Form Tolak STBJ" desktop-mode>
    <template #header-actions>
      ...
      <v-btn
        size="small"
        @click="
          showConfirmation('Konfirmasi Batal', 'Tutup form dan batalkan aksi?', () => router.back())
        "
      >
        Batal
      </v-btn>
      ...
      <div class="left-column">
        <div class="desktop-form-section header-section">
          <v-row dense class="compact-form">
            <v-col cols="6"
              ><v-text-field label="No. Tolak" v-model="header.nomor" readonly filled hide-details
            /></v-col>
            <v-col cols="6"
              ><v-text-field
                label="Tgl. Tolak"
            ...
            <v-col cols="4"
              ><v-text-field
                label="Ke Gudang Prepare"
                v-model="header.gudangRepair"
                readonly
                filled
                hide-details
            /></v-col>
            <v-col cols="8"
              ><v-text-field v-model="header.gudangRepairNama" readonly filled hide-details
            /></v-col>
