...
const selectedRow = computed<QCMaster | null>(() =>
...
const loadDetails = async (newlyExpandedItems: QCMaster[]) => {
  ...
  try {
    ...
    details.value[nomor] = response.data.map((d: QCDetail, index: number) => ({
      ...d,
      no: index + 1,
    }));
  } catch (err) {
  toast.error(getErrorMessage(err, `Gagal memuat detail untuk ${nomor}.`));
  details.value[nomor] = [];
} finally {
    ...
    <div class="browse-content">
      <div class="filter-section">
        ...
        <div class="d-flex align-center ga-2 text-caption me-4">
          <v-icon color="orange-darken-3" icon="mdi-square-rounded" size="small"></v-icon> Status
          Open
        </div>
      ...
      <div class="table-container">
        ...
        >
          ...
          <template #[`item.Nomor`]="{ item }">
            <strong :class="item.Closing === 'N' ? 'text-orange-darken-3' : ''">{{
              item.Nomor
            }}</strong>
          </template>
