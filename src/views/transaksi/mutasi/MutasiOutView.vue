...
const fetchData = async () => {
  ...
  try {
    ...
    const error = err as AxiosError<{ message?: string }>;
    toast.error(error.response?.data?.message || "Gagal memuat data Mutasi Out.");
  } finally {
...
const exportData = async (type: "header" | "detail") => {
  if (type === "header") {
    ...
    try {
      ...
      const response = await api.get<MutasiExportRow[]>("/mutasi-out-form/export-details", {
        params: filters,
      });
      ...
      const dateRange = `Periode : ${formatDateIndo(startDate.value)} s/d ${formatDateIndo(
        endDate.value
      )}`;
  ...
  <PageLayout title="Mutasi Out ke Produksi" desktop-mode icon="mdi-truck-delivery-outline">
    <template #header-actions>
      <v-btn
        ...
        @click="router.push('/transaksi/mutasi/out-produksi/new')"
        >Baru</v-btn
      >
      <v-btn
