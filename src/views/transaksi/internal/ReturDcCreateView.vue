...
const onProductsSelected = async (selectedProducts: any[]) => {
    ...
    if (productsToAdd.length === 0 && selectedProducts.length > 0) return toast.info('Semua produk sudah ada di daftar.');

    try {
        const detailPromises = productsToAdd.map(p => api.get('/retur-dc-form/lookup/product-details', { params: { kode: p.kode, ukuran: p.ukuran, gudang: authStore.user?.cabang } }));
        ...
        addNewRow();
    } catch (error) { toast.error('Gagal memuat detail produk.'); }
};
        ...
        <div class="form-grid-container">
            <div class="left-column">
                <div class="desktop-form-section header-section">
                    <v-row dense>
                        <v-col cols="12"><v-text-field label="No. Retur" v-model="header.nomor" readonly filled
                        ...
                        <v-col cols="12"><v-text-field label="Gudang DC" v-model="header.gudangDc.kode"
                                @click="dialog.gudangSearch = true" prepend-inner-icon="mdi-magnify" readonly
                                variant="outlined" hide-details density="compact" /></v-col>
                        <v-col cols="12"><v-text-field label="Nama Gudang DC" v-model="header.gudangDc.nama" readonly
                    ...
                    <v-data-table :headers="tableHeaders" :items="items" :loading="isLoading"
                        ...
                        <template #item.jumlah="{ item }">
                            <v-text-field v-model.number="item.jumlah" type="number" variant="underlined"
                                ...
                                :rules="[v => v <= item.stok || 'Max stok']" min="0" />
                        </template>
                        ...
                        <template #bottom>
                            <div class="pa-2 text-right"><v-btn size="small" @click="addNewRow"
                                    prepend-icon="mdi-plus">Tambah Baris</v-btn></div>
                        </template>
                    </v-data-table>
                </div>
            </div>
