...
<template>
  <PageLayout title="Update Buffer Stok" desktop-mode icon="mdi-database-sync" max-width="960px">
    <div class="update-container">
      <div class="desktop-form-section action-panel">
        ...
        <v-text-field v-model="pin" type="password" label="PIN Otorisasi" placeholder="Masukkan PIN Anda"
          variant="outlined" density="compact" prepend-inner-icon="mdi-lock-outline" hide-details="auto"></v-text-field>

        <v-divider class="my-5"></v-divider>

        <h3 class="section-title">Pilih Target Update</h3>
        <div class="mb-4">
          <v-checkbox v-model="updateDc" label="Update Cabang DC" density="compact" hide-details></v-checkbox>
          <v-checkbox v-model="updateStore" label="Update Semua Store" density="compact" hide-details></v-checkbox>
        </div>
    ...
    <template #footer>
      <div class="d-flex align-center">
        <v-icon size="x-small" class="me-2">{{ (updateDc || updateStore) ? 'mdi-check-circle' :
