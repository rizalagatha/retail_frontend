...
const handleBarcodeScan = () => {
  ...
  if (itemFound) {
    ...
    if (newQty > itemFound.jumlahKirim) { // <-- ganti 'jumlah' jadi 'jumlahKirim'
    ...
    <div class="form-grid-container">
      <div class="left-column">
        <div class="desktop-form-section header-section">
          <v-row dense>
            <v-col cols="12"><v-text-field label="No. Terima" v-model="header.nomor" readonly filled density="compact"
                hide-details /></v-col>
            <v-col cols="12"><v-text-field label="Tgl. Terima" v-model="header.tanggal" type="date" variant="outlined"
                density="compact" hide-details /></v-col>
            <v-col cols="12"><v-text-field label="No. Kirim (RB)" v-model="header.nomorRb" readonly filled
                density="compact" hide-details /></v-col>
            <v-col cols="12"><v-text-field label="Dari Gudang" v-model="header.gudangAsalNama" readonly filled
                density="compact" hide-details /></v-col>
            <v-col cols="12"><v-textarea label="Keterangan" v-model="header.keterangan" readonly filled rows="3"
                density="compact" hide-details /></v-col>
          </v-row>
        </div>
      </div>
        ...
        <div class="desktop-form-section d-flex flex-column fill-height">
          <v-data-table :headers="tableHeaders" :items="items" :loading="isLoading"
            ...
            <template #[`item.terima`]="{ item }">
              <v-text-field v-model.number="item.terima" type="number" variant="underlined" class="text-end"
                @update:model-value="calculateSelisih(item)" :max="item.jumlahKirim" min="0" density="compact"
                hide-details />
            </template>
