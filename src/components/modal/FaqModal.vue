      ...
      const matchingItems = group.items.filter(
        (item) =>
          ...
          item.a
            ...
            .replace(/<[^>]*>?/gm, "")
            .toLowerCase()
            .includes(keyword) // Cari di text jawaban (strip HTML)
      );
...
<template>
    ...
    <v-card class="rounded-lg d-flex flex-column" style="max-height: 85vh">
      ...
      <div class="px-4 py-3 faq-search">
        <v-text-field
          ...
          label="Ketik pertanyaan (misal: stok, retur, password)..."
          variant="outlined"
          density="compact"
          hide-details
          bg-color="white"
      ...
      <v-card-text class="pa-0 faq-body" style="overflow-y: auto">
        ...
        <div v-else class="pa-2">
          <template v-for="(group, i) in filteredFaq" :key="i">
            <div
              class="d-flex align-center px-4 py-2 mt-2 text-teal-darken-4 font-weight-bold text-uppercase"
              style="font-size: 0.75rem; letter-spacing: 1px"
            >
              <v-icon
                size="small"
                start
                :icon="group.icon || 'mdi-circle-small'"
            ...
            <v-expansion-panels variant="inset" class="mb-2">
              ...
              >
                ...
                <v-expansion-panel-text class="text-body-2 faq-answer pt-2">
                  ...
                  >
                    <v-card
                      v-for="(img_path, k) in item.images"
                      :key="k"
                      variant="outlined"
                    ...
                    >
                      <v-img
                        ...
                        width="180"
                        cover
                        class="bg-grey-lighten-3 cursor-pointer zoom-hover"
                        @click="openZoom(img_path)"
                      >
  ...
  <v-dialog v-model="zoomDialog" max-width="95vw" max-height="95vh" z-index="3000" scrim="black">
    <v-card
      class="bg-black d-flex align-center justify-center position-relative"
    ...
    >
      <v-btn
        ...
        style="z-index: 3001; background-color: rgba(0, 0, 0, 0.5)"
