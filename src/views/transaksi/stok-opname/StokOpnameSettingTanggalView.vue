...
const dialogSetting = reactive({
  ...
  cabangTarget: null as string | null,
});
...
const selectedRow = computed<StokOpnameDate | null>(() =>
...
// --- Methods ---
const handleRowClick = (event: MouseEvent, { item }: { item: StokOpnameDate }) => {
  selected.value = [item];
};
...
const openDeleteDialog = () => {
  ...
  dialogConfirm.text = `Yakin ingin menghapus tanggal stok opname <strong>${format(
    parseISO(selectedRow.value!.tanggal),
    "dd MMMM yyyy"
  )}</strong>?`;
    ...
    <div class="browse-content">
      <div class="table-container">
        <AppDataTable
          ...
          density="compact"
          fixed-header
          :item-value="(item: StokOpnameDate) => `${item.cabang}-${item.tanggal}`"
          return-object
          single-select
    ...
    <v-dialog v-model="dialogSetting.show" max-width="450px" persistent>
      <v-card>
        ...
        <v-card-text class="pt-4">
          <v-select
            v-if="isKdc"
            v-model="dialogSetting.cabangTarget"
            :items="cabangList"
            item-title="nama"
            item-value="kode"
            label="Pilih Cabang"
        ...
        <v-card-actions class="pa-3">
          ...
          <v-btn color="primary" variant="flat" @click="handleSetTanggal" :loading="isLoading"
            >Simpan</v-btn
          >
        </v-card-actions>
      </v-card>
