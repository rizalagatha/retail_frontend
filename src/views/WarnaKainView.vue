...
const fetchData = async () => {
    ...
    selected.value = [];
    try {
        const response = await api.get('/warna-kain');
...
const confirmDelete = async () => {
    ...
    try {
        ...
        const response = await api.delete(`/warna-kain/${encodeURIComponent(warna)}`);
...
<template>
    <PageLayout title="Browse Warna Kain" icon="mdi-palette">
        ...
        <MasterSimpleFormModal v-if="isFormModalVisible" title="Tambah Warna Kain Baru" apiUrl="/warna-kain"
            label1="Kode" field1="Kode" label2="Warna Kain" field2="Warna" @close="isFormModalVisible = false"
            @saved="onDataSaved" />
