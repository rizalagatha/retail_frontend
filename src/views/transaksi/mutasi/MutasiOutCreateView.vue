...
const onSoSelected = async (so: { Nomor: string }) => {
  ...
  try {
    const response = await api.get<SoDetailItem[]>(
      `/mutasi-out-form/lookup/so-details/${so.Nomor}`
    );
...
const save = () => {
  ...
  if (items.value.length === 0 || !items.value.some((i) => i.nama))
...
onMounted(() => {
  ...
  if (!authStore.can(MENU_ID, requiredPermission.value)) {
    toast.error(
      `Anda tidak memiliki izin untuk ${isEditMode.value ? "mengubah" : "membuat"} data Mutasi Out.`
    );
...
<template>
  <PageLayout :title="pageTitle" desktop-mode icon="mdi-package-variant-closed-edit">
    <template #header-actions>
      <v-btn
        v-if="authStore.can(MENU_ID, requiredPermission)"
        size="small"
      ...
      <v-btn
        ...
        @click="showConfirmation(resetForm, 'Batalkan dan kosongkan form?')"
        >Batal</v-btn
      >
      <v-btn
        size="small"
        prepend-icon="mdi-close"
        @click="showConfirmation(closeForm, 'Tutup form?')"
        >Tutup</v-btn
      >
    </template>
          ...
          <v-row dense>
            <v-col cols="12">
              ...
              >
                <template #append-inner>
                  <span v-if="!formHeader.nomor" style="color: #888; font-size: 11px"
                    >&lt;Otomatis&gt;</span
                  >
                </template>
              </v-text-field>
            ...
            <v-col cols="12">
              <v-text-field
                label="Ke Cabang"
                :model-value="
                  formHeader.keCabang ? `${formHeader.keCabang} - ${formHeader.keCabangNama}` : ''
                "
                readonly
