...
const MENU_ID = '46'; // <-- Mohon sesuaikan ID menu untuk modul ini
...
// Watch ini sekarang hanya bertugas untuk memantau perubahan OLEH PENGGUNA
watch(filters, (newFilters, oldFilters) => {
    // Cek ini untuk mencegah pemanggilan ganda saat pertama kali dimuat.
    // Ini memastikan watch hanya berjalan jika ada perubahan nilai yang sebenarnya.
    if (JSON.stringify(newFilters) === JSON.stringify(oldFilters)) {
        return;
    }

    // Saat kode barang dihapus oleh pengguna, hapus juga namanya
...
<template>
    <PageLayout title="Mutasi Antar Store Kirim" :icon="'mdi-store-transfer-outline'">
            ...
            <div class="filter-section">
                ...
                <v-text-field v-model="filters.itemCode" label="Kode Barang" density="compact" hide-details
                    ...
                    <template #append-inner>
                        <v-icon @click="openProductSearch">mdi-magnify</v-icon>
                    </template>
                </v-text-field>
        ...
        <ProductSearchModal v-if="isProductSearchVisible" :gudang="filters.cabang"
            @close="isProductSearchVisible = false" @products-selected="onProductSelected" />
