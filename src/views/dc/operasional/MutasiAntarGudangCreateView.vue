...
const pageTitle = computed(() =>
  isEditMode.value ? "Ubah Mutasi Antar Gudang" : "Buat Mutasi Antar Gudang"
);
...
const loadDataForEdit = async (nomor: string) => {
  try {
    ...
    items.value = data.items.map(
      (item: ApiItem): Item => ({
        ...item,
        id: Math.random(), // id ditambahkan di sini secara aman
      })
    );
    addNewRow();
  } catch (err: unknown) {
...
const fetchGudangOptions = async () => {
  try {
    ...
    // [PERBAIKAN] Ekstrak pesan error
    let msg = "Gagal memuat opsi gudang.";
...
const handleBarcodeScan = async () => {
  ...
  try {
    ...
    const isDuplicate = items.value.some(
      (item) => item.kode === product.kode && item.ukuran === product.ukuran
    );
    if (isDuplicate) {
...
const save = () => {
  ...
  if (!items.value.some((item) => item.kode && item.jumlah > 0))
    ...
    products.forEach((product) => {
      ...
      if (!isDuplicate) {
        ...
        items.value.splice(lastIndex, 0, {
          ...product, // Spread dulu semua isinya
          id: Date.now() + Math.random(), // Lalu timpa ID-nya dengan yang baru di bawah
          jumlah: 1,
        });
      }
    });
  ...
  <PageLayout :title="pageTitle" :menu-id="MENU_ID">
    <template #header-actions>
      ...
      <v-btn
        ...
        @click="
          () => showConfirmation('Konfirmasi Batal', 'Batalkan perubahan?', () => router.back())
        "
      >
        Batal
      </v-btn>
      ...
      <div class="right-column">
        <div class="scanner-wrapper mb-4">
          <v-text-field
            ...
            label="Scan Barcode untuk Menambah Item..."
            variant="outlined"
            density="compact"
    ...
    <MintaBarangSearchModal
      ...
      source="ambil-barang"
      :gudang="header.dariGudang"
      :multi="isModalMultiSelect"
