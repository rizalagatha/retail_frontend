  ...
  <PageLayout title="Surat Jalan ke Store" icon="mdi-truck-delivery">
    <template #header-actions>
      <v-btn v-if="authStore.can(MENU_ID, 'insert')" size="small" color="primary" prepend-icon="mdi-plus"
        @click="router.push({ name: 'SuratJalanCreate' })">Baru</v-btn>
    ...
    <div class="browse-content">
      <div class="filter-section">
        ...
        <v-text-field v-model="filters.kodeBarang" placeholder="Kode Barang (F1)" density="compact" hide-details
          clearable variant="outlined" style="max-width: 200px;" class="ms-4"
        ...
        <v-text-field v-model="filters.namaBarang" placeholder="Nama Barang" density="compact" hide-details readonly
          variant="outlined" class="filter-nama-barang ms-2" style="max-width: 250px;">
        </v-text-field>
      ...
      <div class="table-container">
        <AppDataTable v-model="selected" v-model:expanded="expanded" :headers="masterHeaders" :items="filteredList"
          ...
          <template #[`item.TglTerima`]="{ item }">
            <span v-if="item.TglTerima">{{ format(new Date(item.TglTerima as string), 'dd-MM-yyyy') }}</span>
