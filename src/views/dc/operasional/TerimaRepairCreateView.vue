...
watch(
  ...
  (newItems) => {
    newItems.forEach((item) => {
      const totalAlokasi =
        (item.kbs || 0) + (item.kps || 0) + (item.k01 || 0) + (item.k02 || 0) + (item.k03 || 0);
...
onMounted(async () => {
  ...
  try {
    const response = await api.get(`/terima-repair-form/load-from-kirim`, {
      params: { nomorKirim },
    });
  ...
  <PageLayout title="Terima dari Gudang Repair" desktop-mode>
    <template #header-actions>
      ...
      <v-btn
        ...
        @click="showConfirmation('Konfirmasi Batal', 'Tutup form?', () => router.back())"
        >Batal</v-btn
      >
    </template>
      ...
      <div class="left-column">
        <div class="desktop-form-section header-section">
          <v-row dense class="compact-form">
            <v-col cols="6"
              ><v-text-field label="No. Terima" v-model="header.nomor" readonly filled
            /></v-col>
            <v-col cols="6"
              ><v-text-field
                label="Tgl. Terima"
            ...
            /></v-col>
            <v-col cols="6"
              ><v-text-field label="No. Kirim" v-model="header.nomorKirim" readonly filled
            /></v-col>
            <v-col cols="6"
              ><v-text-field label="Tgl. Kirim" v-model="header.tanggalKirim" readonly filled
            /></v-col>
            <v-col cols="12"
              ><v-text-field label="Dari Gudang" v-model="header.dariGudang" readonly filled
            /></v-col>
            <v-col cols="12"
              ><v-textarea label="Keterangan" v-model="header.keterangan" readonly filled rows="3"
            /></v-col>
          </v-row>
        </div>
