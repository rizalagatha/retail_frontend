...
const selectedRow = computed<ProformaItem | null>(() => isSingleSelected.value ? selected.value[0] : null);
...
const handleCetak = () => {
    ...
    router.push({ name: 'ProformaEdit', params: { id: selectedRow.value!.nomor }, query: { print: 'true' } });
};

const handleDelete = () => {
    if (!canDelete.value) return;
    dialogConfirm.title = 'Konfirmasi Hapus';
    dialogConfirm.text = `Yakin menghapus Proforma Invoice dengan nomor <strong>${selectedRow.value!.nomor}</strong>?`;
...
<template>
    <PageLayout title="Browse Proforma Invoice" :menu-id="MENU_ID">
        <template #header-actions>
            ...
            <v-menu offset-y v-if="authStore.can(MENU_ID, 'export')">
        ...
        <div class="browse-content">
            <div class="filter-section">
                ...
                <v-text-field v-model="filters.endDate" type="date" density="compact" hide-details variant="outlined"
                    style="max-width: 180px;" @change="fetchMasterData" />
                <v-select v-model="filters.cabang" :items="branchOptions" item-title="nama" item-value="kode"
                    label="Cabang" density="compact" hide-details variant="outlined" class="ms-4"
                    style="max-width: 200px;" @update:modelValue="fetchMasterData" />
                <v-spacer />
                <v-btn @click="fetchMasterData" icon="mdi-refresh" variant="text" size="small" :loading="loading" />
            </div>

            <div class="table-container">
                <v-data-table-server v-model="selected" v-model:expanded="expanded" :headers="headers"
                    :items="masterData" :loading="loading" item-value="nomor" density="compact" class="desktop-table"
                    fixed-header show-select show-expand return-object single-select @update:expanded="loadDetails">
                    ...
                    <template #item.closing="{ item }">
                        <v-chip size="x-small" :color="item.closing === 'Y' ? 'red' : 'green'">{{ item.closing
                        }}</v-chip>
                    </template>
                                ...
                                <div class="detail-container">
                                    <div class="detail-table-wrapper">
                                        ...
                                        <v-data-table v-else :headers="detailHeaders" :items="details[item.nomor]"
                                            density="compact" class="detail-table">
