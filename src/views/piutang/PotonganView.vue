...
interface PotonganItem extends PotonganHeader {
    // Digunakan untuk seleksi tabel, mewarisi semua properti Header
}
...
const expanded = ref<PotonganHeader[]>([]);
...
const selectedRow = computed<PotonganItem | null>(() =>
    isSingleSelected.value ? selected.value[0] as PotonganItem : null
);
...
const loadDetails = async (newlyExpandedItems: PotonganItem[]) => {
    ...
    try {
        // Ganti endpoint menjadi /potongan/details
...
const getRowTextColor = (item: PotonganItem) => {
        ...
        <template #header-actions>
            ...
            <v-menu offset-y>
                <template v-slot:activator="{ props }">
                    <v-btn size="small" color="teal" prepend-icon="mdi-file-excel" v-bind="props" :disabled="loading || masterData.length === 0">
                        Export
                    </v-btn>
                </template>
...
.detail-table-wrapper {
    max-height: 300px; /* Batasi tinggi container detail */
    overflow-y: auto;
    margin: 8px 0;
